...
implements AdvancementTabGenerator {
    ...
    public void accept(RegistryWrapper.WrapperLookup registries, Consumer<AdvancementEntry> exporter) {
        ...
        AdvancementEntry lv7 = VanillaAdventureTabAdvancementGenerator.createKillMobAdvancements(lv4, exporter, lv, MONSTERS);
    ...
    private static Advancement.Builder requireAllExclusiveTrimmedArmor(Advancement.Builder builder) {
        ...
        VanillaRecipeGenerator.streamSmithingTemplates().filter(template -> set.contains(template.template())).forEach(templatex -> builder.criterion("armor_trimmed_" + String.valueOf(templatex.id().getValue()), RecipeCraftedCriterion.Conditions.create(templatex.id())));
        return builder;
    }
    ...
    private static Advancement.Builder requireTrimmedArmor(Advancement.Builder builder) {
        ...
        VanillaRecipeGenerator.streamSmithingTemplates().map(VanillaRecipeGenerator.SmithingTemplate::id).forEach(template -> builder.criterion("armor_trimmed_" + String.valueOf(template.getValue()), RecipeCraftedCriterion.Conditions.create(template)));
        return builder;
    }
