...
public class DataCommandStorage {
    ...
    private PersistentState.Type<PersistentState> getPersistentStateType(String namespace) {
        return new PersistentState.Type<PersistentState>(() -> this.createStorage(namespace), (nbt, registries) -> this.createStorage(namespace).readNbt((NbtCompound)nbt), DataFixTypes.SAVED_DATA_COMMAND_STORAGE);
    }
