...
public class StructureTemplate {
    ...
    public boolean place(ServerWorldAccess world, BlockPos pos, BlockPos pivot, StructurePlacementData placementData, Random random, int flags) {
        ...
        for (StructureBlockInfo lv2 : list5) {
            ...
            if (lv != null && !lv.contains(lv3)) continue;
            FluidState fluidState = placementData.shouldApplyWaterlogging() ? world.getFluidState(lv3) : null;
    ...
    public static JigsawBlockEntity.Joint readJoint(NbtCompound nbt, BlockState state) {
        return JigsawBlockEntity.Joint.CODEC.byId(nbt.getString("joint"), () -> JigsawBlock.getFacing(state).getAxis().isHorizontal() ? JigsawBlockEntity.Joint.ALIGNED : JigsawBlockEntity.Joint.ROLLABLE);
    }
    ...
    public record JigsawBlockInfo(StructureBlockInfo info, JigsawBlockEntity.Joint jointType, Identifier name, Identifier pool, Identifier target, int placementPriority, int selectionPriority) {
        public static JigsawBlockInfo of(StructureBlockInfo structureBlockInfo) {
            ...
            return new JigsawBlockInfo(structureBlockInfo, StructureTemplate.readJoint(lv, structureBlockInfo.state()), Identifier.of(lv.getString("name")), Identifier.of(lv.getString("pool")), Identifier.of(lv.getString("target")), lv.getInt("placement_priority"), lv.getInt("selection_priority"));
        }
