...
public class OceanMonumentGenerator {
    ...
    static class PieceSetting {
        ...
        public boolean hasEntranceConnection(int distance) {
            ...
            for (int j = 0; j < 6; ++j) {
                if (this.neighbors[j] == null || !this.neighborPresences[j] || this.neighbors[j].entranceDistance == distance || !this.neighbors[j].hasEntranceConnection(distance)) continue;
                return true;
            }
            return false;
        }
    ...
    extends Piece {
        ...
        public void generate(StructureWorldAccess world, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox chunkBox, ChunkPos chunkPos, BlockPos pivot) {
            ...
            if (this.setting.roomIndex / 25 > 0) {
                this.generateVerticalConnection(world, chunkBox, 0, 8, lv.neighborPresences[Direction.DOWN.getId()]);
                this.generateVerticalConnection(world, chunkBox, 0, 0, lv2.neighborPresences[Direction.DOWN.getId()]);
            }
    ...
    extends Piece {
        ...
        public void generate(StructureWorldAccess world, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox chunkBox, ChunkPos chunkPos, BlockPos pivot) {
            ...
            if (this.setting.roomIndex / 25 > 0) {
                this.generateVerticalConnection(world, chunkBox, 8, 0, lv.neighborPresences[Direction.DOWN.getId()]);
                this.generateVerticalConnection(world, chunkBox, 0, 0, lv2.neighborPresences[Direction.DOWN.getId()]);
            }
            if (lv3.neighbors[Direction.UP.getId()] == null) {
    ...
    extends Piece {
        ...
        public void generate(StructureWorldAccess world, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox chunkBox, ChunkPos chunkPos, BlockPos pivot) {
            ...
            boolean bl2 = bl = this.roomType != 0 && random.nextBoolean() && !this.setting.neighborPresences[Direction.DOWN.getId()] && !this.setting.neighborPresences[Direction.UP.getId()] && this.setting.countNeighbors() > 1;
            if (this.roomType == 0) {
    ...
    extends Piece {
        ...
        private List<PieceSetting> setupPieces(Random random) {
            ...
            for (PieceSetting lv6 : objectArrayList) {
                ...
                for (m = 0; r < 2 && m < 5; ++m) {
                    ...
                    if (lv6.hasEntranceConnection(q++) && lv6.neighbors[n].hasEntranceConnection(q++)) {
                        ++r;
                        continue;
                    }
        ...
        private void generateWing(boolean side, int start, StructureWorldAccess world, Random random, BlockBox box) {
            ...
            if (this.boxIntersects(box, start, 0, start + 23, 20)) {
                ...
                for (k = 0; k < 4; ++k) {
                    this.fillWithOutline(world, box, start + k, k + 1, k, start + k, k + 1, 20, PRISMARINE_BRICKS, PRISMARINE_BRICKS, false);
                    this.fillWithOutline(world, box, start + k + 7, k + 5, k + 7, start + k + 7, k + 5, 20, PRISMARINE_BRICKS, PRISMARINE_BRICKS, false);
                    this.fillWithOutline(world, box, start + 17 - k, k + 5, k + 7, start + 17 - k, k + 5, 20, PRISMARINE_BRICKS, PRISMARINE_BRICKS, false);
                    this.fillWithOutline(world, box, start + 24 - k, k + 1, k, start + 24 - k, k + 1, 20, PRISMARINE_BRICKS, PRISMARINE_BRICKS, false);
                    this.fillWithOutline(world, box, start + k + 1, k + 1, k, start + 23 - k, k + 1, k, PRISMARINE_BRICKS, PRISMARINE_BRICKS, false);
                    this.fillWithOutline(world, box, start + k + 8, k + 5, k + 7, start + 16 - k, k + 5, k + 7, PRISMARINE_BRICKS, PRISMARINE_BRICKS, false);
                }
                ...
                for (m = 0; m < 4; ++m) {
                    int n = side ? start + 24 - (17 - m * 3) : start + 17 - m * 3;
    ...
    extends StructurePiece {
        ...
        private static BlockBox createBox(Direction orientation, PieceSetting setting, int x, int y, int z) {
            ...
            int n = l / 5 % 5;
            int o = l / 25;
            BlockBox lv = Piece.createBox(0, 0, 0, orientation, x * 8, y * 4, z * 8);
            switch (orientation) {
                case NORTH: {
                    lv.move(m * 8, o * 4, -(n + z) * 8 + 1);
                    break;
                }
                case SOUTH: {
                    lv.move(m * 8, o * 4, n * 8);
        ...
        protected void spawnElderGuardian(StructureWorldAccess world, BlockBox box, int x, int y, int z) {
            ...
            if (box.contains(lv) && (lv2 = EntityType.ELDER_GUARDIAN.create(world.toServerWorld(), SpawnReason.STRUCTURE)) != null) {
