...
public interface StructurePoolAliasBinding {
    ...
    public static RandomStructurePoolAliasBinding random(String alias, DataPool<String> targets) {
        ...
        targets.getEntries().forEach(target -> lv.add(StructurePools.ofVanilla((String)target.data()), target.getWeight().getValue()));
        return StructurePoolAliasBinding.random(StructurePools.ofVanilla(alias), lv.build());
    }
