...
extends ChunkManager {
    ...
    public WorldChunk loadChunkFromPacket(int x, int z, PacketByteBuf buf, NbtCompound nbt, Consumer<ChunkData.BlockEntityVisitor> consumer) {
        ...
        if (!ClientChunkManager.positionEquals(lv, x, z)) {
            ...
            lv.loadFromPacket(buf, nbt, consumer);
            this.chunks.refreshSections(lv);
        }
    ...
    final class ClientChunkMap {
        ...
        private void unloadChunkSections(WorldChunk chunk) {
            ...
            for (int i = 0; i < lvs.length; ++i) {
                ChunkPos lv = chunk.getPos();
                this.activeSections.remove(ChunkSectionPos.asLong(lv.x, chunk.sectionIndexToCoord(i), lv.z));
            }
        }
        ...
        private void loadChunkSections(WorldChunk chunk) {
            ...
            for (int i = 0; i < lvs.length; ++i) {
                ...
                this.activeSections.add(ChunkSectionPos.asLong(lv2.x, chunk.sectionIndexToCoord(i), lv2.z));
            }
        }
        ...
        void refreshSections(WorldChunk chunk) {
            ...
            ChunkSection[] lvs = chunk.getSectionArray();
            for (int i = 0; i < lvs.length; ++i) {
                ChunkSection lv2 = lvs[i];
                long l = ChunkSectionPos.asLong(lv.x, chunk.sectionIndexToCoord(i), lv.z);
                if (lv2.isEmpty()) {
                    this.activeSections.add(l);
                    continue;
                }
