...
implements DynamicTexture {
    ...
    public BakedGlyph bake(RenderableGlyph glyph) {
        ...
        if (lv != null) {
            ...
            float g = 256.0f;
            float h = 0.01f;
            return new BakedGlyph(this.textRenderLayers, ((float)lv.x + 0.01f) / 256.0f, ((float)lv.x - 0.01f + (float)glyph.getWidth()) / 256.0f, ((float)lv.y + 0.01f) / 256.0f, ((float)lv.y - 0.01f + (float)glyph.getHeight()) / 256.0f, glyph.getXMin(), glyph.getXMax(), glyph.getYMin(), glyph.getYMax());
        }
        return null;
    }
