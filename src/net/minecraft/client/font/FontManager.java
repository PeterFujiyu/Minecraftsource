...
AutoCloseable {
    ...
    public CompletableFuture<Void> reload(ResourceReloader.Synchronizer synchronizer, ResourceManager manager, Executor prepareExecutor, Executor applyExecutor) {
        return ((CompletableFuture)this.loadIndex(manager, prepareExecutor).thenCompose(synchronizer::whenPrepared)).thenAcceptAsync(index -> this.reload((ProviderIndex)index, Profilers.get()), applyExecutor);
    }
