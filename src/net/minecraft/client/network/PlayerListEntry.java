...
public class PlayerListEntry {
    ...
    private static java.util.function.Supplier<SkinTextures> texturesSupplier(GameProfile profile) {
        ...
        CompletableFuture<Optional<SkinTextures>> completableFuture = lv2.fetchSkinTextures(profile);
        boolean bl = !lv.uuidEquals(profile.getId());
        SkinTextures lv3 = DefaultSkinHelper.getSkinTextures(profile);
        return () -> {
            SkinTextures lv = completableFuture.getNow(Optional.empty()).orElse(lv3);
            if (bl && !lv.secure()) {
                return lv3;
            }
            return lv;
