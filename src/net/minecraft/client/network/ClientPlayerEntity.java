...
extends AbstractClientPlayerEntity {
    ...
    private void sendMovementPackets() {
        ...
        if (this.isCamera()) {
            ...
            if (bl && bl2) {
                this.networkHandler.sendPacket(new PlayerMoveC2SPacket.Full(this.getX(), this.getY(), this.getZ(), this.getYaw(), this.getPitch(), this.isOnGround(), this.horizontalCollision));
            } else if (bl) {
    ...
    public void init() {
        ...
        if (this.getWorld() != null) {
            for (double d = this.getY(); d > (double)this.getWorld().getBottomY() && d <= (double)this.getWorld().getTopYInclusive(); d += 1.0) {
    ...
    private boolean shouldStopSprinting() {
        return this.isGliding() || this.isBlind() || this.shouldSlowDown() || this.hasVehicle() && !this.isRidingCamel() || this.isUsingItem() && !this.hasVehicle() && !this.isSubmergedInWater();
    }
    ...
    private boolean canStartSprinting() {
        return !(this.isSprinting() || !this.isWalking() || !this.canSprint() || this.isUsingItem() || this.isBlind() || this.hasVehicle() && !this.canVehicleSprint(this.getVehicle()) || this.isGliding() || this.shouldSlowDown() && !this.isSubmergedInWater());
    }
