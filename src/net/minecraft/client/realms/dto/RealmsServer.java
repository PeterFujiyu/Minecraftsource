...
import net.minecraft.client.realms.dto.RealmsWorldSettings;
...
extends ValueObject {
    ...
    public static RealmsServer parse(JsonObject node) {
        ...
        try {
            ...
            lv.description = JsonUtils.getNullableStringOr("motd", node, "");
            ...
            lv.hardcore = JsonUtils.getBooleanOr("isHardcore", node, false);
            lv.gameMode = JsonUtils.getIntOr("gameMode", node, -1);
    ...
    private static Map<Integer, RealmsWorldOptions> parseSlots(JsonArray json) {
        ...
        for (JsonElement jsonElement : json) {
            try {
                ...
                RealmsWorldSettings lv = RealmsServer.parseSettings(jsonObject.get("settings"));
                RealmsWorldOptions lv2 = jsonElement2 == null ? RealmsWorldOptions.getDefaults() : RealmsWorldOptions.parse(jsonElement2.getAsJsonObject(), lv);
    ...
    private static RealmsWorldSettings parseSettings(JsonElement json) {
        ...
        if (json.isJsonArray()) {
            for (JsonElement jsonElement2 : json.getAsJsonArray()) {
                ...
                bl = RealmsServer.isSet(jsonObject, "hardcore", bl);
            }
        }
    ...
    private static boolean isSet(JsonObject json, String name, boolean defaultValue) {
        String string2 = JsonUtils.getNullableStringOr("name", json, null);
        if (string2 != null && string2.equals(name)) {
            return JsonUtils.getBooleanOr("value", json, defaultValue);
        }
        return defaultValue;
    }
    ...
    public String getWorldName(int slotId) {
        if (this.name == null) {
            return this.slots.get(slotId).getSlotName(slotId);
        }
