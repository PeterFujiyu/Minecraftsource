...
extends RealmsScreen {
    ...
    public void init() {
        ...
        this.addDrawableChild(ButtonWidget.builder(Text.translatable("mco.configure.world.subscription.extend"), button -> ConfirmLinkScreen.open((Screen)this, Urls.getExtendJavaRealmsUrl(this.serverData.remoteSubscriptionId, this.client.getSession().getUuidOrNull()))).dimensions(this.width / 2 - 100, RealmsSubscriptionInfoScreen.row(6), 200, 20).build());
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawTextWithShadow(this.textRenderer, SUBSCRIPTION_START_LABEL_TEXT, k, RealmsSubscriptionInfoScreen.row(0), Colors.LIGHT_GRAY);
        context.drawTextWithShadow(this.textRenderer, this.startDate, k, RealmsSubscriptionInfoScreen.row(1), Colors.WHITE);
        if (this.type == Subscription.SubscriptionType.NORMAL) {
            context.drawTextWithShadow(this.textRenderer, TIME_LEFT_LABEL_TEXT, k, RealmsSubscriptionInfoScreen.row(3), Colors.LIGHT_GRAY);
        } else if (this.type == Subscription.SubscriptionType.RECURRING) {
            context.drawTextWithShadow(this.textRenderer, DAYS_LEFT_LABEL_TEXT, k, RealmsSubscriptionInfoScreen.row(3), Colors.LIGHT_GRAY);
        }
        context.drawTextWithShadow(this.textRenderer, this.daysLeft, k, RealmsSubscriptionInfoScreen.row(4), Colors.WHITE);
    }
