...
extends RealmsScreen {
    ...
    void handle(boolean accepted) {
        ...
        if (e instanceof PendingInvitationSelectionListEntry) {
            PendingInvitationSelectionListEntry lv = (PendingInvitationSelectionListEntry)e;
            String string = lv.mPendingInvite.invitationId;
            CompletableFuture.supplyAsync(() -> {
                ...
                if (result.booleanValue()) {
                    this.pendingInvitationSelectionList.remove(lv);
                    this.updateButtonStates();
    ...
    void updateButtonStates() {
        PendingInvitationSelectionListEntry lv = (PendingInvitationSelectionListEntry)this.pendingInvitationSelectionList.getSelectedOrNull();
        this.acceptButton.visible = lv != null;
        this.rejectButton.visible = lv != null;
    }
    ...
    extends AlwaysSelectedEntryListWidget.Entry<PendingInvitationSelectionListEntry> {
        ...
        private void renderPendingInvitationItem(DrawContext context, PendingInvite invite, int x, int y, int mouseX, int mouseY) {
            ...
            context.drawTextWithShadow(RealmsPendingInvitesScreen.this.textRenderer, invite.worldOwnerName, x + 38, y + 12, 0x6C6C6C);
            context.drawTextWithShadow(RealmsPendingInvitesScreen.this.textRenderer, RealmsUtil.convertToAgePresentation(invite.date), x + 38, y + 24, 0x6C6C6C);
        ...
        extends RealmsAcceptRejectButton {
            ...
            protected void render(DrawContext context, int x, int y, boolean showTooltip) {
                context.drawGuiTexture(RenderLayer::getGuiTextured, showTooltip ? ACCEPT_HIGHLIGHTED_ICON_TEXTURE : ACCEPT_ICON_TEXTURE, x, y, 18, 18);
                if (showTooltip) {
        ...
        extends RealmsAcceptRejectButton {
            ...
            @Override
            protected void render(DrawContext context, int x, int y, boolean showTooltip) {
                context.drawGuiTexture(RenderLayer::getGuiTextured, showTooltip ? REJECT_HIGHLIGHTED_ICON_TEXTURE : REJECT_ICON_TEXTURE, x, y, 18, 18);
                if (showTooltip) {
