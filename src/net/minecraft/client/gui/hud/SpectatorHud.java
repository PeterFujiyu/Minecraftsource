...
implements SpectatorMenuCloseCallback {
    ...
    protected void renderSpectatorMenu(DrawContext context, float height, int x, int y, SpectatorMenuState state) {
        ...
        context.drawGuiTexture(RenderLayer::getGuiTextured, HOTBAR_TEXTURE, x - 91, y, 182, 22, k);
        if (state.getSelectedSlot() >= 0) {
            context.drawGuiTexture(RenderLayer::getGuiTextured, HOTBAR_SELECTION_TEXTURE, x - 91 - 1 + state.getSelectedSlot() * 20, y - 1, 24, 23, k);
        }
        for (int l = 0; l < 9; ++l) {
    ...
    private void renderSpectatorCommand(DrawContext arg, int slot, int x, float y, float height, SpectatorMenuCommand arg2) {
        if (arg2 != SpectatorMenu.BLANK_COMMAND) {
            arg.getMatrices().push();
            arg.getMatrices().translate(x, y, 0.0f);
            float h = arg2.isEnabled() ? 1.0f : 0.25f;
            arg2.renderIcon(arg, h, height);
            arg.getMatrices().pop();
            int k = (int)(height * 255.0f);
            if (k > 3 && arg2.isEnabled()) {
                Text lv = this.client.options.hotbarKeys[slot].getBoundKeyLocalizedText();
                arg.drawTextWithShadow(this.client.textRenderer, lv, x + 19 - 2 - this.client.textRenderer.getWidth(lv), (int)y + 6 + 3, 0xFFFFFF + (k << 24));
            }
        }
    }
    ...
    public void render(DrawContext context) {
        ...
        if (i > 3 && this.spectatorMenu != null) {
            ...
            if (lv2 != null) {
                ...
                context.drawTextWithBackground(this.client.textRenderer, lv2, k, l, j, ColorHelper.withAlpha(i, Colors.WHITE));
            }
        }
    }
