...
public class PlayerListHud {
    ...
    public void render(DrawContext context, int scaledWindowWidth, Scoreboard scoreboard, @Nullable ScoreboardObjective objective) {
        ...
        for (int x = 0; x < o; ++x) {
            ...
            if (bl) {
                ...
                PlayerSkinDrawer.draw(context, lv9.getSkinTextures().texture(), z, aa, 8, lv9.shouldShowHat(), bl22, -1);
                z += 9;
            }
    ...
    protected void renderLatencyIcon(DrawContext context, int width, int x, int y, PlayerListEntry entry) {
        ...
        context.drawGuiTexture(RenderLayer::getGuiTextured, lv, x + width - 11, y, 10, 8);
    ...
    private void renderHearts(int y, int left, int right, UUID uuid, DrawContext context, int score) {
        ...
        for (p = m; p < n; ++p) {
            context.drawGuiTexture(RenderLayer::getGuiTextured, lv4, left + p * o, y, 9, 9);
        }
        for (p = 0; p < m; ++p) {
            context.drawGuiTexture(RenderLayer::getGuiTextured, lv4, left + p * o, y, 9, 9);
            if (bl) {
                if (p * 2 + 1 < lv.getPrevScore()) {
                    context.drawGuiTexture(RenderLayer::getGuiTextured, FULL_HEART_BLINKING_TEXTURE, left + p * o, y, 9, 9);
                }
                if (p * 2 + 1 == lv.getPrevScore()) {
                    context.drawGuiTexture(RenderLayer::getGuiTextured, HALF_HEART_BLINKING_TEXTURE, left + p * o, y, 9, 9);
                }
            }
            if (p * 2 + 1 < score) {
                context.drawGuiTexture(RenderLayer::getGuiTextured, p >= 10 ? ABSORBING_FULL_HEART_BLINKING_TEXTURE : FULL_HEART_TEXTURE, left + p * o, y, 9, 9);
            }
            if (p * 2 + 1 != score) continue;
            context.drawGuiTexture(RenderLayer::getGuiTextured, p >= 10 ? ABSORBING_HALF_HEART_BLINKING_TEXTURE : HALF_HEART_TEXTURE, left + p * o, y, 9, 9);
        }
    }
