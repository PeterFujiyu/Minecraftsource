...
public class ChatHud {
    ...
    public void render(DrawContext context, int currentTick, int mouseX, int mouseY, boolean focused) {
        ...
        for (int u = 0; u + this.scrolledLines < this.visibleMessages.size() && u < l; ++u) {
            ...
            ChatHudLine.Visible lv2 = this.visibleMessages.get(v);
            if (lv2 == null || (w = currentTick - lv2.addedTime()) >= 200 && !focused) continue;
            ...
            MessageIndicator lv3 = lv2.indicator();
            if (lv3 != null) {
                int ac = lv3.indicatorColor() | x << 24;
                context.fill(-4, aa - r, -2, aa, ac);
                if (v == q && lv3.icon() != null) {
                    int ad = this.getIndicatorX(lv2);
                    int ae = ab + this.client.textRenderer.fontHeight;
                    this.drawIndicatorIcon(context, ad, ae, lv3.icon());
                }
            }
            context.getMatrices().push();
            context.getMatrices().translate(0.0f, 0.0f, 50.0f);
            context.drawTextWithShadow(this.client.textRenderer, lv2.content(), 0, ab, ColorHelper.withAlpha(x, Colors.WHITE));
