...
public class InGameHud {
    ...
    private void renderMiscOverlays(DrawContext context, RenderTickCounter tickCounter) {
        ...
        if (this.client.options.getPerspective().isFirstPerson()) {
            if (this.client.player.isUsingSpyglass()) {
                ...
                for (EquipmentSlot lv : EquipmentSlot.values()) {
                    ...
                    if (lv3 == null || lv3.slot() != lv || !lv3.cameraOverlay().isPresent()) continue;
                    this.renderOverlay(context, lv3.cameraOverlay().get().withPath(overlayTexture -> "textures/" + overlayTexture + ".png"), 1.0f);
                }
            }
        }
        ...
        if ((g = MathHelper.lerp(tickCounter.getTickDelta(false), this.client.player.prevNauseaIntensity, this.client.player.nauseaIntensity)) > 0.0f) {
    ...
    private void renderStatusEffectOverlay(DrawContext context, RenderTickCounter tickCounter) {
        ...
        for (StatusEffectInstance lv2 : Ordering.natural().reverse().sortedCopy(collection)) {
            ...
            if (lv2.isAmbient()) {
                ...
                if (lv2.isDurationBelow(200)) {
                    int m = lv2.getDuration();
                    n = 10 - m / 20;
            ...
            float g = f;
            list.add(() -> {
                int k = ColorHelper.getWhite(g);
                context.drawSpriteStretched(RenderLayer::getGuiTextured, lv4, n + 3, o + 3, 18, 18, k);
            });
        }
    ...
    private void renderHotbar(DrawContext context, RenderTickCounter tickCounter) {
        ...
        context.drawGuiTexture(RenderLayer::getGuiTextured, HOTBAR_SELECTION_TEXTURE, i - 91 - 1 + lv.getInventory().selectedSlot * 20, context.getScaledWindowHeight() - 22 - 1, 24, 23);
        if (!lv2.isEmpty()) {
            if (lv3 == Arm.LEFT) {
    ...
    private void renderMountJumpBar(JumpingMount mount, DrawContext context, int x) {
        ...
        context.drawGuiTexture(RenderLayer::getGuiTextured, JUMP_BAR_BACKGROUND_TEXTURE, x, l, 182, 5);
        if (mount.getJumpCooldown() > 0) {
            context.drawGuiTexture(RenderLayer::getGuiTextured, JUMP_BAR_COOLDOWN_TEXTURE, x, l, 182, 5);
        } else if (k > 0) {
            context.drawGuiTexture(RenderLayer::getGuiTextured, JUMP_BAR_PROGRESS_TEXTURE, 182, 5, 0, 0, x, l, k, 5);
        }
        Profilers.get().pop();
    }
    ...
    private void renderExperienceBar(DrawContext context, int x) {
        ...
        if (j > 0) {
            ...
            context.drawGuiTexture(RenderLayer::getGuiTextured, EXPERIENCE_BAR_BACKGROUND_TEXTURE, x, m, 182, 5);
            if (l > 0) {
                context.drawGuiTexture(RenderLayer::getGuiTextured, EXPERIENCE_BAR_PROGRESS_TEXTURE, 182, 5, 0, 0, x, m, l, 5);
            }
        }
        Profilers.get().pop();
    }
    ...
    private void renderScoreboardSidebar(DrawContext arg, ScoreboardObjective objective) {
        int i;
        ...
        int o = arg.getScaledWindowHeight() / 2 + n / 3;
        int p = 3;
        int q = arg.getScaledWindowWidth() - l - 3;
        int r = arg.getScaledWindowWidth() - 3 + 2;
        ...
        arg.fill(q - 2, u - this.getTextRenderer().fontHeight - 1, r, u - 1, t);
        arg.fill(q - 2, u - 1, r, o, s);
        arg.drawText(this.getTextRenderer(), lv3, q + l / 2 - i / 2, u - this.getTextRenderer().fontHeight, Colors.WHITE, false);
        for (int v = 0; v < m; ++v) {
            SidebarEntry lv5 = lvs[v];
            int w = o - (m - v) * this.getTextRenderer().fontHeight;
            arg.drawText(this.getTextRenderer(), lv5.name, q, w, Colors.WHITE, false);
            arg.drawText(this.getTextRenderer(), lv5.score, r - lv5.scoreWidth, w, Colors.WHITE, false);
        }
    }
    ...
    private static void renderArmor(DrawContext context, PlayerEntity player, int i, int j, int k, int x) {
        ...
        for (int o = 0; o < 10; ++o) {
            int p = x + o * 8;
            if (o * 2 + 1 < m) {
                context.drawGuiTexture(RenderLayer::getGuiTextured, ARMOR_FULL_TEXTURE, p, n, 9, 9);
            }
            if (o * 2 + 1 == m) {
                context.drawGuiTexture(RenderLayer::getGuiTextured, ARMOR_HALF_TEXTURE, p, n, 9, 9);
            }
            if (o * 2 + 1 <= m) continue;
            context.drawGuiTexture(RenderLayer::getGuiTextured, ARMOR_EMPTY_TEXTURE, p, n, 9, 9);
        }
    }
    ...
    private void renderAirBubbles(DrawContext context, PlayerEntity player, int heartCount, int top, int left) {
        ...
        if (bl || m < l) {
            ...
            for (int q = 1; q <= 10; ++q) {
                int r = left - (q - 1) * 8 - 9;
                if (q <= n) {
                    context.drawGuiTexture(RenderLayer::getGuiTextured, AIR_TEXTURE, r, top, 9, 9);
                    continue;
                }
                if (bl2 && q == o && bl) {
                    context.drawGuiTexture(RenderLayer::getGuiTextured, AIR_BURSTING_TEXTURE, r, top, 9, 9);
                    this.playBurstSound(q, player, p);
                    continue;
                }
                if (q <= 10 - p) continue;
                int s = p == 10 && this.ticks % 2 == 0 ? this.random.nextInt(2) : 0;
                context.drawGuiTexture(RenderLayer::getGuiTextured, AIR_EMPTY_TEXTURE, r, top + s, 9, 9);
            }
        }
    }
    ...
    private void renderFood(DrawContext context, PlayerEntity player, int top, int right) {
        ...
        for (int l = 0; l < 10; ++l) {
            ...
            int n = right - l * 8 - 9;
            context.drawGuiTexture(RenderLayer::getGuiTextured, lv2, n, m, 9, 9);
            if (l * 2 + 1 < k) {
                context.drawGuiTexture(RenderLayer::getGuiTextured, lv4, n, m, 9, 9);
            }
            if (l * 2 + 1 != k) continue;
            context.drawGuiTexture(RenderLayer::getGuiTextured, lv3, n, m, 9, 9);
        }
    }
    ...
    private void renderSpyglassOverlay(DrawContext context, float scale) {
        ...
        int n = l + j;
        int o = m + k;
        context.drawTexture(RenderLayer::getGuiTextured, SPYGLASS_SCOPE, l, m, 0.0f, 0.0f, j, k, j, k);
    ...
    private void renderVignetteOverlay(DrawContext context, @Nullable Entity entity) {
        ...
        context.drawTexture(RenderLayer::getVignette, VIGNETTE_TEXTURE, 0, 0, 0.0f, 0.0f, context.getScaledWindowWidth(), context.getScaledWindowHeight(), context.getScaledWindowWidth(), context.getScaledWindowHeight(), i);
    }
    ...
    private void renderPortalOverlay(DrawContext context, float nauseaStrength) {
        ...
        context.drawSpriteStretched(RenderLayer::getGuiTexturedOverlay, lv, 0, 0, context.getScaledWindowWidth(), context.getScaledWindowHeight(), i);
    }
    ...
    private void renderNauseaOverlay(DrawContext context, float nauseaStrength) {
        ...
        context.drawTexture(arg -> RenderLayer.getGuiNauseaOverlay(), NAUSEA_TEXTURE, 0, 0, 0.0f, 0.0f, i, j, i, j, ColorHelper.fromFloats(1.0f, h, k, l));
        context.getMatrices().pop();
    }
