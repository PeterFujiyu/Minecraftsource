    ...
    extends AlwaysSelectedEntryListWidget<Entry> {
        ...
        protected void renderHeader(DrawContext context, int x, int y) {
            ...
            for (k = 0; k < this.headerIconTextures.length; ++k) {
                ...
                context.drawGuiTexture(RenderLayer::getGuiTextured, lv, x + this.getIconX(k) - 18, y + 1, 18, 18);
            }
            if (this.selectedStatType != null) {
                k = this.getIconX(this.getHeaderIndex(this.selectedStatType)) - 36;
                lv = this.listOrder == 1 ? SORT_UP_TEXTURE : SORT_DOWN_TEXTURE;
                context.drawGuiTexture(RenderLayer::getGuiTextured, lv, x + k, y + 1, 18, 18);
            }
            for (k = 0; k < this.headerIconTextures.length; ++k) {
                int l = this.selectedHeaderColumn == k ? 1 : 0;
                context.drawGuiTexture(RenderLayer::getGuiTextured, this.headerIconTextures[k], x + this.getIconX(k) - 18 + l, y + 1 + l, 18, 18);
            }
        }
        ...
        protected void renderDecorations(DrawContext context, int mouseX, int mouseY) {
            ...
            if (lv != null) {
                ...
                Item lv2 = lv.getItem();
                context.drawTooltip(StatsScreen.this.textRenderer, lv2.getName(), mouseX, mouseY, lv2.getComponents().get(DataComponentTypes.TOOLTIP_STYLE));
            } else {
                Text lv3 = null;
                int l = mouseX - k;
                for (int m = 0; m < this.headerIconTextures.length; ++m) {
                    int n = this.getIconX(m);
            ...
            public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
                ...
                if (StatsScreen.this.itemStats != null) {
                    ...
                    for (p = 0; p < StatsScreen.this.itemStats.blockStatTypes.size(); ++p) {
                        ...
                        this.render(context, lv2, x + ItemStatsListWidget.this.getIconX(p), y + entryHeight / 2 - ((StatsScreen)StatsScreen.this).textRenderer.fontHeight / 2, index % 2 == 0);
                    }
                    for (p = 0; p < StatsScreen.this.itemStats.itemStatTypes.size(); ++p) {
                        this.render(context, StatsScreen.this.itemStats.itemStatTypes.get(p).getOrCreateStat(this.item), x + ItemStatsListWidget.this.getIconX(p + StatsScreen.this.itemStats.blockStatTypes.size()), y + entryHeight / 2 - ((StatsScreen)StatsScreen.this).textRenderer.fontHeight / 2, index % 2 == 0);
                    }
                }
            }
