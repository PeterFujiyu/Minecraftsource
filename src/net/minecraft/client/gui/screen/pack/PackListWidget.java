...
extends AlwaysSelectedEntryListWidget<ResourcePackEntry> {
    ...
    protected void renderHeader(DrawContext context, int x, int y) {
        ...
        context.drawTextWithShadow(this.client.textRenderer, lv, x + this.width / 2 - this.client.textRenderer.getWidth(lv) / 2, Math.min(this.getY() + 3, y), Colors.WHITE);
    }
    ...
    extends AlwaysSelectedEntryListWidget.Entry<ResourcePackEntry> {
        ...
        public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            if (!lv.isCompatible()) {
                int p = x + entryWidth - 3 - (this.widget.overflows() ? 7 : 0);
            ...
            if (this.isSelectable() && (this.client.options.getTouchscreen().getValue().booleanValue() || hovered || this.widget.getSelectedOrNull() == this && this.widget.isFocused())) {
                ...
                if (this.pack.canBeEnabled()) {
                    if (q < 32) {
                        context.drawGuiTexture(RenderLayer::getGuiTextured, SELECT_HIGHLIGHTED_TEXTURE, x, y, 32, 32);
                    } else {
                        context.drawGuiTexture(RenderLayer::getGuiTextured, SELECT_TEXTURE, x, y, 32, 32);
                    }
                } else {
                    if (this.pack.canBeDisabled()) {
                        if (q < 16) {
                            context.drawGuiTexture(RenderLayer::getGuiTextured, UNSELECT_HIGHLIGHTED_TEXTURE, x, y, 32, 32);
                        } else {
                            context.drawGuiTexture(RenderLayer::getGuiTextured, UNSELECT_TEXTURE, x, y, 32, 32);
                        }
                    }
