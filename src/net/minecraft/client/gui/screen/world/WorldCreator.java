...
public class WorldCreator {
    ...
    private void updateWorldTypeLists() {
        RegistryWrapper.Impl lv = this.getGeneratorOptionsHolder().getCombinedRegistryManager().getOrThrow(RegistryKeys.WORLD_PRESET);
        this.normalWorldTypes.clear();
        this.normalWorldTypes.addAll(WorldCreator.getWorldPresetList((Registry<WorldPreset>)lv, WorldPresetTags.NORMAL).orElseGet(() -> ((Registry)lv).streamEntries().map(WorldType::new).toList()));
        this.extendedWorldTypes.clear();
        this.extendedWorldTypes.addAll((Collection<WorldType>)WorldCreator.getWorldPresetList((Registry<WorldPreset>)lv, WorldPresetTags.EXTENDED).orElse(this.normalWorldTypes));
        RegistryEntry<WorldPreset> lv2 = this.worldType.preset();
        if (lv2 != null) {
            boolean bl;
            WorldType lv3 = WorldCreator.getWorldPreset(this.getGeneratorOptionsHolder(), lv2.getKey()).map(WorldType::new).orElse(this.normalWorldTypes.getFirst());
            boolean bl2 = bl = LevelScreenProvider.WORLD_PRESET_TO_SCREEN_PROVIDER.get(lv2.getKey()) != null;
            if (bl) {
                this.worldType = lv3;
            } else {
                this.setWorldType(lv3);
            }
        }
    }
