...
extends HandledScreen<CartographyTableScreenHandler> {
        ...
        if (lv3 != null) {
            ...
            if (lv4 != null) {
                if (lv4.locked) {
                    bl4 = true;
                    if (bl2 || bl3) {
                        context.drawGuiTexture(RenderLayer::getGuiTextured, ERROR_TEXTURE, k + 35, l + 31, 28, 21);
                    }
                }
                if (bl2 && lv4.scale >= 4) {
                    bl4 = true;
    ...
    private void drawMap(DrawContext context, @Nullable MapIdComponent mapId, @Nullable MapState mapState, int x, int y, float scale) {
        if (mapId != null && mapState != null) {
            ...
            MapRenderer lv = this.client.getMapRenderer();
            lv.update(mapId, mapState, this.mapRenderState);
            context.draw(vertexConsumers -> lv.draw(this.mapRenderState, context.getMatrices(), (VertexConsumerProvider)vertexConsumers, true, 0xF000F0));
            context.getMatrices().pop();
        }
    }
}
