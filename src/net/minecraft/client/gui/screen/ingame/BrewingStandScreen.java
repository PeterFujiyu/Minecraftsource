...
extends HandledScreen<BrewingStandScreenHandler> {
    ...
    protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        ...
        if (n > 0) {
            context.drawGuiTexture(RenderLayer::getGuiTextured, FUEL_LENGTH_TEXTURE, 18, 4, 0, 0, k + 60, l + 44, n, 4);
        }
        if ((o = ((BrewingStandScreenHandler)this.handler).getBrewTime()) > 0) {
            int p = (int)(28.0f * (1.0f - (float)o / 400.0f));
            if (p > 0) {
                context.drawGuiTexture(RenderLayer::getGuiTextured, BREW_PROGRESS_TEXTURE, 9, 28, 0, 0, k + 97, l + 16, 9, p);
            }
            if ((p = BUBBLE_PROGRESS[o / 2 % 7]) > 0) {
                context.drawGuiTexture(RenderLayer::getGuiTextured, BUBBLES_TEXTURE, 12, 29, 0, 29 - p, k + 63, l + 14 + 29 - p, 12, p);
            }
        }
    }
}
