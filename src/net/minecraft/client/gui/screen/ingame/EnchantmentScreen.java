...
extends HandledScreen<EnchantmentScreenHandler> {
    ...
    protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        ...
        for (int n = 0; n < 3; ++n) {
            ...
            if (q == 0) {
                context.drawGuiTexture(RenderLayer::getGuiTextured, ENCHANTMENT_SLOT_DISABLED_TEXTURE, o, l + 14 + 19 * n, 108, 19);
                continue;
            }
            String string = "" + q;
            ...
            if (!(m >= n + 1 && this.client.player.experienceLevel >= q || this.client.player.getAbilities().creativeMode)) {
                context.drawGuiTexture(RenderLayer::getGuiTextured, ENCHANTMENT_SLOT_DISABLED_TEXTURE, o, l + 14 + 19 * n, 108, 19);
                context.drawGuiTexture(RenderLayer::getGuiTextured, LEVEL_DISABLED_TEXTURES[n], o + 1, l + 15 + 19 * n, 16, 16);
                context.drawWrappedText(this.textRenderer, lv, p, l + 16 + 19 * n, r, (s & 0xFEFEFE) >> 1, false);
                s = 4226832;
            } else {
                int t = mouseX - (k + 60);
                int u = mouseY - (l + 14 + 19 * n);
                if (t >= 0 && u >= 0 && t < 108 && u < 19) {
                    context.drawGuiTexture(RenderLayer::getGuiTextured, ENCHANTMENT_SLOT_HIGHLIGHTED_TEXTURE, o, l + 14 + 19 * n, 108, 19);
                    s = 0xFFFF80;
                } else {
                    context.drawGuiTexture(RenderLayer::getGuiTextured, ENCHANTMENT_SLOT_TEXTURE, o, l + 14 + 19 * n, 108, 19);
                }
                context.drawGuiTexture(RenderLayer::getGuiTextured, LEVEL_TEXTURES[n], o + 1, l + 15 + 19 * n, 16, 16);
    ...
    private void drawBook(DrawContext context, int x, int y, float delta) {
        ...
        context.draw(vertexConsumers -> {
            VertexConsumer lv = vertexConsumers.getBuffer(this.BOOK_MODEL.getLayer(BOOK_TEXTURE));
            this.BOOK_MODEL.render(context.getMatrices(), lv, 0xF000F0, OverlayTexture.DEFAULT_UV);
        });
        context.draw();
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        float g = this.client.getRenderTickCounter().getTickDelta(false);
