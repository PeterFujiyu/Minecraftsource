...
extends HandledScreen<LoomScreenHandler> {
    ...
    protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        ...
        if (!lv.hasStack()) {
            context.drawGuiTexture(RenderLayer::getGuiTextured, BANNER_SLOT_TEXTURE, k + lv.x, l + lv.y, 16, 16);
        }
        if (!lv2.hasStack()) {
            context.drawGuiTexture(RenderLayer::getGuiTextured, DYE_SLOT_TEXTURE, k + lv2.x, l + lv2.y, 16, 16);
        }
        if (!lv3.hasStack()) {
            context.drawGuiTexture(RenderLayer::getGuiTextured, PATTERN_SLOT_TEXTURE, k + lv3.x, l + lv3.y, 16, 16);
        }
        int m = (int)(41.0f * this.scrollPosition);
        Identifier lv5 = this.canApplyDyePattern ? SCROLLER_TEXTURE : SCROLLER_DISABLED_TEXTURE;
        context.drawGuiTexture(RenderLayer::getGuiTextured, lv5, k + 119, l + 13 + m, 12, 15);
        context.draw();
        ...
        if (this.bannerPatterns != null && !this.hasTooManyPatterns) {
            ...
            context.draw(vertexConsumers -> BannerBlockEntityRenderer.renderCanvas(context.getMatrices(), vertexConsumers, 0xF000F0, OverlayTexture.DEFAULT_UV, this.bannerField, ModelBaker.BANNER_BASE, true, lv6, this.bannerPatterns));
            context.getMatrices().pop();
        } else if (this.hasTooManyPatterns) {
            context.drawGuiTexture(RenderLayer::getGuiTextured, ERROR_TEXTURE, k + lv4.x - 5, l + lv4.y - 5, 26, 26);
        }
        if (this.canApplyDyePattern) {
            ...
            block0: for (int p = 0; p < 4; ++p) {
                for (int q = 0; q < 4; ++q) {
                    ...
                    context.drawGuiTexture(RenderLayer::getGuiTextured, lv7, t, u, 14, 14);
    ...
    private void drawBanner(DrawContext context, RegistryEntry<BannerPattern> pattern, int x, int y) {
        ...
        context.draw(vertexConsumers -> BannerBlockEntityRenderer.renderCanvas(lv, vertexConsumers, 0xF000F0, OverlayTexture.DEFAULT_UV, this.bannerField, ModelBaker.BANNER_BASE, true, DyeColor.GRAY, lv2));
        lv.pop();
        context.draw();
    }
