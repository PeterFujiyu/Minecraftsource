...
implements ScreenHandlerProvider<T> {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (!this.touchDropReturningStack.isEmpty()) {
            ...
            int p = this.touchDropX + (int)((float)n * g);
            int q = this.touchDropY + (int)((float)o * g);
            this.drawItem(context, this.touchDropReturningStack, p, q, null);
        }
    ...
    protected void drawMouseoverTooltip(DrawContext drawContext, int x, int y) {
        ...
        if (((ScreenHandler)this.handler).getCursorStack().isEmpty() || this.isItemTooltipSticky(lv)) {
            drawContext.drawTooltip(this.textRenderer, this.getTooltipFromItem(lv), lv.getTooltipData(), x, y, lv.get(DataComponentTypes.TOOLTIP_STYLE));
        }
    }
    ...
    protected void drawSlot(DrawContext context, Slot slot) {
        ...
        if (lv.isEmpty() && slot.isEnabled() && (lv3 = slot.getBackgroundSprite()) != null) {
            context.drawGuiTexture(RenderLayer::getGuiTextured, lv3, i, j, 16, 16);
            bl2 = true;
        }
        if (!bl2) {
            if (bl) {
