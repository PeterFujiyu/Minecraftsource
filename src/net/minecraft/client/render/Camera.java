...
public class Camera {
    ...
    public void update(BlockView area, Entity focusedEntity, boolean thirdPerson, boolean inverseView, float tickDelta) {
        ...
        if (focusedEntity.hasVehicle() && (object = focusedEntity.getVehicle()) instanceof MinecartEntity && (object = (lv = (MinecartEntity)object).getController()) instanceof ExperimentalMinecartController && (lv2 = (ExperimentalMinecartController)object).hasCurrentLerpSteps()) {
            Vec3d lv3 = lv.getPassengerRidingPos(focusedEntity).subtract(lv.getPos()).subtract(focusedEntity.getVehicleAttachmentPos(lv)).add(new Vec3d(0.0, MathHelper.lerp(tickDelta, this.lastCameraY, this.cameraY), 0.0));
            this.setRotation(focusedEntity.getYaw(tickDelta), focusedEntity.getPitch(tickDelta));
            this.setPos(lv2.getLerpedPosition(tickDelta).add(lv3));
        } else {
            this.setRotation(focusedEntity.getYaw(tickDelta), focusedEntity.getPitch(tickDelta));
        ...
        if (thirdPerson) {
            ...
            Direction lv5 = ((LivingEntity)focusedEntity).getSleepingDirection();
            this.setRotation(lv5 != null ? lv5.getPositiveHorizontalDegrees() - 180.0f : 0.0f, 0.0f);
