    ...
    public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, double cameraX, double cameraY, double cameraZ) {
        ...
        for (Listener lv4 : this.listeners) {
            lv4.getPos(lv).ifPresent(pos -> {
                ...
                DebugRenderer.drawVoxelShapeOutlines(matrices, lv3, VoxelShapes.cuboid(new Box(g, h, i, j, k, l)), -cameraX, -cameraY, -cameraZ, 1.0f, 1.0f, 0.0f, 0.35f, true);
            });
        }
        VertexConsumer lv5 = vertexConsumers.getBuffer(RenderLayer.getDebugFilledBox());
        for (Listener lv6 : this.listeners) {
            lv6.getPos(lv).ifPresent(pos -> VertexRendering.drawFilledBox(matrices, lv5, pos.getX() - 0.25 - cameraX, pos.getY() - cameraY, pos.getZ() - 0.25 - cameraZ, pos.getX() + 0.25 - cameraX, pos.getY() - cameraY + 1.0, pos.getZ() + 0.25 - cameraZ, 1.0f, 1.0f, 0.0f, 0.35f));
        }
        for (Listener lv6 : this.listeners) {
