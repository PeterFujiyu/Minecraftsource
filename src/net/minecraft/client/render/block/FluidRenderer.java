...
public class FluidRenderer {
    ...
    private static boolean isSideCovered(Direction arg, float f, BlockState arg2) {
        VoxelShape lv = arg2.getCullingFace(arg.getOpposite());
        if (lv == VoxelShapes.empty()) {
            return false;
        }
        if (lv == VoxelShapes.fullCube()) {
            boolean bl = f == 1.0f;
            return arg != Direction.UP || bl;
        }
        VoxelShape lv2 = VoxelShapes.cuboid(0.0, 0.0, 0.0, 1.0, f, 1.0);
        return VoxelShapes.isSideCovered(lv2, lv, arg);
    }

    private static boolean method_3344(Direction arg, float f, BlockState arg2) {
        return FluidRenderer.isSideCovered(arg, f, arg2);
    }

    private static boolean isOppositeSideCovered(BlockState arg, Direction arg2) {
        return FluidRenderer.isSideCovered(arg2.getOpposite(), 1.0f, arg);
    }

    public static boolean shouldRenderSide(FluidState arg, BlockState arg2, Direction arg3, FluidState arg4) {
        return !FluidRenderer.isOppositeSideCovered(arg2, arg3) && !FluidRenderer.isSameFluid(arg, arg4);
    }
    ...
    public void render(BlockRenderView world, BlockPos pos, VertexConsumer vertexConsumer, BlockState blockState, FluidState fluidState) {
        ...
        boolean bl3 = FluidRenderer.shouldRenderSide(fluidState, blockState, Direction.DOWN, lv2) && !FluidRenderer.method_3344(Direction.DOWN, 0.8888889f, lv);
        boolean bl4 = FluidRenderer.shouldRenderSide(fluidState, blockState, Direction.NORTH, lv6);
        boolean bl5 = FluidRenderer.shouldRenderSide(fluidState, blockState, Direction.SOUTH, lv8);
        boolean bl6 = FluidRenderer.shouldRenderSide(fluidState, blockState, Direction.WEST, lv10);
        boolean bl7 = FluidRenderer.shouldRenderSide(fluidState, blockState, Direction.EAST, lv12);
        ...
        if (bl2 && !FluidRenderer.method_3344(Direction.UP, Math.min(Math.min(p, r), Math.min(q, o)), lv3)) {
            float ag;
            float af;
            float y;
            p -= 0.001f;
        ...
        for (Direction lv16 : Direction.Type.HORIZONTAL) {
            ...
            }) || FluidRenderer.method_3344(lv16, Math.max(ad, y), world.getBlockState(pos.offset(lv16)))) continue;
