...
public class BlockModelRenderer {
    ...
    public void renderSmooth(BlockRenderView world, BakedModel model, BlockState state, BlockPos pos, MatrixStack matrices, VertexConsumer vertexConsumer, boolean cull, Random random, long seed, int overlay) {
        ...
        for (Direction lv3 : DIRECTIONS) {
            ...
            if (cull && !Block.shouldDrawSide(state, world.getBlockState(lv2), lv3)) continue;
    ...
    public void renderFlat(BlockRenderView world, BakedModel model, BlockState state, BlockPos pos, MatrixStack matrices, VertexConsumer vertexConsumer, boolean cull, Random random, long seed, int overlay) {
        ...
        for (Direction lv2 : DIRECTIONS) {
            ...
            if (cull && !Block.shouldDrawSide(state, world.getBlockState(lv), lv2)) continue;
    ...
    private void renderQuad(BlockRenderView world, BlockState state, BlockPos pos, VertexConsumer vertexConsumer, MatrixStack.Entry matrixEntry, BakedQuad quad, float brightness0, float brightness1, float brightness2, float brightness3, int light0, int light1, int light2, int light3, int overlay) {
        ...
        float p;
        if (quad.hasTint()) {
            int o = this.colors.getColor(state, world, pos, quad.getTintIndex());
    ...
    static class AmbientOcclusionCalculator {
        ...
        public void apply(BlockRenderView world, BlockState state, BlockPos pos, Direction arg4, float[] fs, BitSet bitSet, boolean bl) {
            float x;
            int u;
            float t;
            int s;
            ...
            BlockState lv13;
            boolean bl5;
            BlockPos lv = bitSet.get(0) ? pos.offset(arg4) : pos;
            ...
            BlockState lv9 = world.getBlockState(lv3.set((Vec3i)lv, lv2.faces[0]).move(arg4));
            boolean bl2 = !lv9.shouldBlockVision(world, lv3) || lv9.getOpacity() == 0;
            BlockState lv10 = world.getBlockState(lv3.set((Vec3i)lv, lv2.faces[1]).move(arg4));
            boolean bl3 = !lv10.shouldBlockVision(world, lv3) || lv10.getOpacity() == 0;
            BlockState lv11 = world.getBlockState(lv3.set((Vec3i)lv, lv2.faces[2]).move(arg4));
            boolean bl4 = !lv11.shouldBlockVision(world, lv3) || lv11.getOpacity() == 0;
            BlockState lv12 = world.getBlockState(lv3.set((Vec3i)lv, lv2.faces[3]).move(arg4));
            boolean bl6 = bl5 = !lv12.shouldBlockVision(world, lv3) || lv12.getOpacity() == 0;
            if (bl4 || bl2) {
            ...
            lv3.set((Vec3i)pos, arg4);
            BlockState lv14 = world.getBlockState(lv3);
            if (bitSet.get(0) || !lv14.isOpaqueFullCube()) {
                v = lv4.getInt(lv14, world, lv3);
            }
            float w = bitSet.get(0) ? lv4.getFloat(world.getBlockState(lv), world, lv) : lv4.getFloat(world.getBlockState(pos), world, pos);
            Translation lv15 = Translation.getTranslations(arg4);
            if (!bitSet.get(1) || !lv2.nonCubicWeight) {
                ...
                z = (h + g + r + w) * 0.25f;
                aa = (m + g + t + w) * 0.25f;
                float ab = fs[lv2.field_4192[0].shape] * fs[lv2.field_4192[1].shape];
                float ac = fs[lv2.field_4192[2].shape] * fs[lv2.field_4192[3].shape];
                float ad = fs[lv2.field_4192[4].shape] * fs[lv2.field_4192[5].shape];
                float ae = fs[lv2.field_4192[6].shape] * fs[lv2.field_4192[7].shape];
                float af = fs[lv2.field_4185[0].shape] * fs[lv2.field_4185[1].shape];
                float ag = fs[lv2.field_4185[2].shape] * fs[lv2.field_4185[3].shape];
                float ah = fs[lv2.field_4185[4].shape] * fs[lv2.field_4185[5].shape];
                float ai = fs[lv2.field_4185[6].shape] * fs[lv2.field_4185[7].shape];
                float aj = fs[lv2.field_4180[0].shape] * fs[lv2.field_4180[1].shape];
                float ak = fs[lv2.field_4180[2].shape] * fs[lv2.field_4180[3].shape];
                float al = fs[lv2.field_4180[4].shape] * fs[lv2.field_4180[5].shape];
                float am = fs[lv2.field_4180[6].shape] * fs[lv2.field_4180[7].shape];
                float an = fs[lv2.field_4188[0].shape] * fs[lv2.field_4188[1].shape];
                float ao = fs[lv2.field_4188[2].shape] * fs[lv2.field_4188[3].shape];
                float ap = fs[lv2.field_4188[4].shape] * fs[lv2.field_4188[5].shape];
                float aq = fs[lv2.field_4188[6].shape] * fs[lv2.field_4188[7].shape];
                this.brightness[lv15.firstCorner] = Math.clamp(x * ab + y * ac + z * ad + aa * ae, 0.0f, 1.0f);
                this.brightness[lv15.secondCorner] = Math.clamp(x * af + y * ag + z * ah + aa * ai, 0.0f, 1.0f);
                this.brightness[lv15.thirdCorner] = Math.clamp(x * aj + y * ak + z * al + aa * am, 0.0f, 1.0f);
                this.brightness[lv15.fourthCorner] = Math.clamp(x * an + y * ao + z * ap + aa * aq, 0.0f, 1.0f);
