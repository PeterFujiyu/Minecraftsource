...
extends AbstractSignBlockEntityRenderer {
    ...
    private final Map<WoodType, SignModelPair> typeToModelPair = WoodType.stream().collect(ImmutableMap.toImmutableMap(signType -> signType, signType -> new SignModelPair(SignBlockEntityRenderer.createSignModel(ctx.getLoadedEntityModels(), signType, true), SignBlockEntityRenderer.createSignModel(ctx.getLoadedEntityModels(), signType, false))));
