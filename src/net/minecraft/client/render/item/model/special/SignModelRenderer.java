...
implements SimpleSpecialModelRenderer {
    ...
    public record Unbaked(WoodType woodType, Optional<Identifier> texture) implements SpecialModelRenderer.Unbaked
    {
        ...
        public SpecialModelRenderer<?> bake(LoadedEntityModels entityModels) {
            Model lv = SignBlockEntityRenderer.createSignModel(entityModels, this.woodType, true);
            SpriteIdentifier lv2 = this.texture.map(TexturedRenderLayers::createSignTextureId).orElseGet(() -> TexturedRenderLayers.getSignTextureId(this.woodType));
            return new SignModelRenderer(lv, lv2);
        }
    }
}
