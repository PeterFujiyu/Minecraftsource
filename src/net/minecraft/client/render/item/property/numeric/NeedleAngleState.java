...
public abstract class NeedleAngleState {
    ...
    public float getValue(ItemStack stack, @Nullable ClientWorld world, @Nullable LivingEntity user, int seed) {
        World world2;
        Entity lv;
        Entity entity = lv = user != null ? user : stack.getHolder();
        if (lv == null) {
            return 0.0f;
        }
        if (world == null && (world2 = lv.getWorld()) instanceof ClientWorld) {
            ClientWorld lv2;
            world = lv2 = (ClientWorld)world2;
        }
        if (world == null) {
            return 0.0f;
        }
        return this.getAngle(stack, world, seed, lv);
    }
