...
implements UnbakedModel {
    ...
    private BakedModel create(ModelTextures textures, SpriteGetter spriteGetter, ModelBakeSettings settings, boolean ambientOcclusion, boolean isSideLit, ModelTransformation transformation) {
        String string;
        SpriteIdentifier lv2;
        ModelTextures.Textures.Builder lv = new ModelTextures.Textures.Builder();
        ArrayList<ModelElement> list = new ArrayList<ModelElement>();
        for (int i = 0; i < LAYERS.size() && (lv2 = textures.get(string = LAYERS.get(i))) != null; ++i) {
            lv.addSprite(string, lv2);
            SpriteContents lv3 = spriteGetter.get(lv2).getContents();
            list.addAll(this.addLayerElements(i, string, lv3));
        }
    ...
    private List<ModelElement> addLayerElements(int layer, String key, SpriteContents sprite) {
        Map<Direction, ModelElementFace> map = Map.of(Direction.SOUTH, new ModelElementFace(null, layer, key, new ModelElementTexture(new float[]{0.0f, 0.0f, 16.0f, 16.0f}, 0)), Direction.NORTH, new ModelElementFace(null, layer, key, new ModelElementTexture(new float[]{16.0f, 0.0f, 0.0f, 16.0f}, 0)));
        ArrayList<ModelElement> list = new ArrayList<ModelElement>();
    ...
    private List<ModelElement> addSubComponents(SpriteContents sprite, String key, int layer) {
        ...
        for (Frame lv : this.getFrames(sprite)) {
            ...
            j = 16.0f - j;
            l = 16.0f - l;
            Map<Direction, ModelElementFace> map = Map.of(lv2.getDirection(), new ModelElementFace(null, layer, key, new ModelElementTexture(new float[]{m *= q, o *= r, n *= q, p *= r}, 0)));
            switch (lv2.ordinal()) {
                case 0: {
