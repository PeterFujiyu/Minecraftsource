...
public class BakedQuadFactory {
    ...
    public static BakedQuad bake(Vector3f from, Vector3f to, ModelElementFace face, Sprite sprite, Direction direction, ModelBakeSettings settings, @Nullable ModelRotation rotation, boolean shade, int lightEmission) {
        ModelElementTexture lv = face.textureData();
        if (settings.isUvLocked()) {
            lv = BakedQuadFactory.uvLock(face.textureData(), direction, settings.getRotation());
        }
        ...
        int[] is = BakedQuadFactory.packVertexData(lv, sprite, direction, BakedQuadFactory.getPositionMatrix(from, to), settings.getRotation(), rotation);
        ...
        return new BakedQuad(is, face.tintIndex(), lv2, sprite, shade, lightEmission);
    }
    ...
    private static int[] packVertexData(ModelElementTexture texture, Sprite sprite, Direction direction, float[] positionMatrix, AffineTransformation orientation, @Nullable ModelRotation rotation) {
        int[] is = new int[32];
        for (int i = 0; i < 4; ++i) {
            BakedQuadFactory.packVertexData(is, i, direction, texture, positionMatrix, sprite, orientation, rotation);
        }
        return is;
    }
    ...
    private static void packVertexData(int[] vertices, int cornerIndex, Direction direction, ModelElementTexture texture, float[] positionMatrix, Sprite sprite, AffineTransformation orientation, @Nullable ModelRotation rotation) {
        ...
        BakedQuadFactory.rotateVertex(vector3f, rotation);
        BakedQuadFactory.transformVertex(vector3f, orientation);
        BakedQuadFactory.packVertexData(vertices, cornerIndex, vector3f, sprite, texture);
    }
    ...
    private static void packVertexData(int[] vertices, int cornerIndex, Vector3f pos, Sprite sprite, ModelElementTexture texture) {
        ...
        vertices[j + 3] = -1;
        vertices[j + 4] = Float.floatToRawIntBits(sprite.getFrameU(texture.getU(cornerIndex) / 16.0f));
        vertices[j + 4 + 1] = Float.floatToRawIntBits(sprite.getFrameV(texture.getV(cornerIndex) / 16.0f));
    }
    ...
    private static void rotateVertex(Vector3f vertex, @Nullable ModelRotation rotation) {
        ...
        BakedQuadFactory.transformVertex(vertex, new Vector3f(rotation.origin()), new Matrix4f().rotation(quaternionf), vector3f3);
    }
    ...
    private static void transformVertex(Vector3f vertex, AffineTransformation transformation) {
        ...
        BakedQuadFactory.transformVertex(vertex, new Vector3f(0.5f, 0.5f, 0.5f), transformation.getMatrix(), new Vector3f(1.0f, 1.0f, 1.0f));
    }
