        ...
        if (lv != null) {
            try {
                if (action == 1 || action == 2) {
                    ...
                    if (lv.keyPressed(key, scancode, modifiers)) {
                        return;
                    }
                } else if (action == 0 && lv.keyReleased(key, scancode, modifiers)) {
                    return;
                }
            } catch (Throwable throwable) {
                CrashReport lv2 = CrashReport.create(throwable, "keyPressed event handler");
                lv.addCrashReportSection(lv2);
                CrashReportSection lv3 = lv2.addElement("Key");
                lv3.add("Key", key);
                lv3.add("Scancode", scancode);
                lv3.add("Mods", modifiers);
    ...
    private void onChar(long window, int codePoint, int modifiers) {
        ...
        try {
            if (Character.isBmpCodePoint(codePoint)) {
                ...
                lv.charTyped(Character.highSurrogate(codePoint), modifiers);
                lv.charTyped(Character.lowSurrogate(codePoint), modifiers);
            }
        } catch (Throwable throwable) {
            CrashReport lv2 = CrashReport.create(throwable, "charTyped event handler");
            lv.addCrashReportSection(lv2);
            CrashReportSection lv3 = lv2.addElement("Key");
            lv3.add("Codepoint", codePoint);
            lv3.add("Mods", modifiers);
            throw new CrashException(lv2);
        }
    }
