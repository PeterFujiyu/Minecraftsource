...
ChunkLoadingManager {
    ...
    private CompletableFuture<Chunk> loadChunk(ChunkPos pos) {
        CompletionStage completableFuture = this.getUpdatedChunkNbt(pos).thenApplyAsync(optional -> optional.map(arg2 -> {
            SerializedChunk lv = SerializedChunk.fromNbt(this.world, this.world.getRegistryManager(), arg2);
            if (lv == null) {
    ...
    private Chunk getProtoChunk(ChunkPos chunkPos) {
        this.markAsProtoChunk(chunkPos);
        return new ProtoChunk(chunkPos, UpgradeData.NO_UPGRADE_DATA, this.world, (Registry<Biome>)this.world.getRegistryManager().getOrThrow(RegistryKeys.BIOME), null);
    }
    ...
    private boolean isLevelChunk(ChunkPos pos) {
        ...
        ChunkType lv2 = SerializedChunk.getChunkType(lv);
    ...
    void forEachTickedChunk(Consumer<ChunkHolder> callback) {
        ...
        while (longIterator.hasNext()) {
            long l = longIterator.nextLong();
            ChunkHolder lv = this.chunkHolders.get(l);
            if (lv == null || !this.isAnyPlayerTicking(lv.getPos())) continue;
            callback.accept(lv);
        }
    }
