...
implements AutoCloseable {
    ...
    protected static URL resolveEndpoint(URI uri, @Nullable JsonObject endpoints, String key, String defaultPath) throws MalformedURLException {
        String string3 = AbstractTextFilterer.getEndpointPath(endpoints, key, defaultPath);
    ...
    protected FilterMask createFilterMask(String raw, JsonArray redactedTextIndex, HashIgnorer hashIgnorer) {
        ...
        FilterMask lv = new FilterMask(raw.length());
        for (int i = 0; i < redactedTextIndex.size(); ++i) {
            lv.markFiltered(redactedTextIndex.get(i).getAsInt());
        }
        return lv;
    }
    ...
    implements TextStream {
        ...
        protected StreamImpl(GameProfile gameProfile) {
            ...
            SimpleConsecutiveExecutor lv = new SimpleConsecutiveExecutor(AbstractTextFilterer.this.threadPool, "chat stream for " + gameProfile.getName());
            this.executor = lv::send;
        }

        @Override
        public CompletableFuture<List<FilteredMessage>> filterTexts(List<String> texts) {
            List list2 = texts.stream().map(text -> AbstractTextFilterer.this.filter(this.gameProfile, (String)text, AbstractTextFilterer.this.hashIgnorer, this.executor)).collect(ImmutableList.toImmutableList());
