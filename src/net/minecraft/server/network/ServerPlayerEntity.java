...
extends PlayerEntity {
    ...
    public BlockPos getWorldSpawnPos(ServerWorld world, BlockPos basePos) {
        ...
        if (world.getDimension().hasSkyLight() && world.getServer().getSaveProperties().getGameMode() != GameMode.ADVENTURE) {
            ...
            for (int p = 0; p < k; ++p) {
                ...
                try {
                    lv2 = SpawnLocating.findOverworldSpawn(world, t, u);
                    if (lv2 == null || !this.canSpawnIn(world, lv.offset(lv2.toBottomCenterPos()))) continue;
                    return lv2;
                } catch (Exception exception) {
                    int v = p;
                    int w = i;
                    CrashReport lv3 = CrashReport.create(exception, "Searching for spawn");
                    CrashReportSection lv4 = lv3.addElement("Spawn Lookup");
                    lv4.add("Origin", basePos::toString);
                    lv4.add("Radius", () -> Integer.toString(w));
                    lv4.add("Candidate", () -> "[" + t + "," + u + "]");
                    lv4.add("Progress", () -> v + " out of " + k);
                    throw new CrashException(lv3);
                }
            }
            lv2 = basePos;
        }
        while (!this.canSpawnIn(world, lv.offset(lv2.toBottomCenterPos())) && lv2.getY() < world.getTopYInclusive()) {
            lv2 = lv2.up();
        }
        while (this.canSpawnIn(world, lv.offset(lv2.down().toBottomCenterPos())) && lv2.getY() > world.getBottomY() + 1) {
            lv2 = lv2.down();
        }
        return lv2;
    }
    ...
    public void readCustomDataFromNbt(NbtCompound nbt) {
        ...
        if (nbt.contains("recipeBook", NbtElement.COMPOUND_TYPE)) {
            this.recipeBook.readNbt(nbt.getCompound("recipeBook"), recipeKey -> this.server.getRecipeManager().get((RegistryKey<Recipe<?>>)recipeKey).isPresent());
        }
        if (this.isSleeping()) {
            this.wakeUp();
        }
    ...
    private void writeRootVehicle(NbtCompound nbt) {
        ...
        if (lv2 != null && lv != this && lv.hasPlayerRider()) {
            ...
            lv.saveNbt(lv4);
            lv3.putUuid("Attach", lv2.getUuid());
            lv3.put("Entity", lv4);
            nbt.put("RootVehicle", lv3);
        }
    }

    public void readRootVehicle(Optional<NbtCompound> nbt) {
        World world;
        if (nbt.isPresent() && nbt.get().contains("RootVehicle", NbtElement.COMPOUND_TYPE) && (world = this.getWorld()) instanceof ServerWorld) {
            ServerWorld lv = (ServerWorld)world;
            NbtCompound lv2 = nbt.get().getCompound("RootVehicle");
            Entity lv3 = EntityType.loadEntityWithPassengers(lv2.getCompound("Entity"), lv, SpawnReason.LOAD, entity -> {
            ...
            UUID uUID = lv2.containsUuid("Attach") ? lv2.getUuid("Attach") : null;
            if (lv3.getUuid().equals(uUID)) {
                this.startRiding(lv3, true);
            } else {
                for (Entity lv4 : lv3.getPassengersDeep()) {
                    if (!lv4.getUuid().equals(uUID)) continue;
                    this.startRiding(lv4, true);
                    break;
                }
            }
            if (!this.hasVehicle()) {
    ...
    private void writeEnderPearls(NbtCompound nbt) {
        if (!this.enderPearls.isEmpty()) {
            ...
            for (EnderPearlEntity lv2 : this.enderPearls) {
                ...
                NbtCompound lv3 = new NbtCompound();
                lv2.saveNbt(lv3);
                Identifier.CODEC.encodeStart(NbtOps.INSTANCE, lv2.getWorld().getRegistryKey().getValue()).resultOrPartial(LOGGER::error).ifPresent(dimension -> lv3.put(ENDER_PEARLS_DIMENSION_KEY, (NbtElement)dimension));
                lv.add(lv3);
            }
            nbt.put(ENDER_PEARLS_KEY, lv);
        }
    }

    public void readEnderPearls(Optional<NbtCompound> nbt) {
        NbtElement lv;
        if (nbt.isPresent() && nbt.get().contains(ENDER_PEARLS_KEY, NbtElement.LIST_TYPE) && (lv = nbt.get().get(ENDER_PEARLS_KEY)) instanceof NbtList) {
            NbtList lv2 = (NbtList)lv;
            lv2.forEach(enderPearlNbt -> {
                NbtCompound lv;
                if (enderPearlNbt instanceof NbtCompound && (lv = (NbtCompound)enderPearlNbt).contains(ENDER_PEARLS_DIMENSION_KEY)) {
                    Optional optional = World.CODEC.parse(NbtOps.INSTANCE, lv.get(ENDER_PEARLS_DIMENSION_KEY)).resultOrPartial(LOGGER::error);
                    if (optional.isEmpty()) {
                        LOGGER.warn("No dimension defined for ender pearl, skipping");
                        return;
                    }
                    ServerWorld lv2 = this.getWorld().getServer().getWorld((RegistryKey)optional.get());
                    if (lv2 != null) {
                        Entity lv3 = EntityType.loadEntityWithPassengers(lv, lv2, SpawnReason.LOAD, enderPearl -> {
                            if (!lv2.tryLoadEntity((Entity)enderPearl)) {
                                return null;
                            }
                            return enderPearl;
                        });
                        if (lv3 != null) {
                            ServerPlayerEntity.addEnderPearlTicket(lv2, lv3.getChunkPos());
                        } else {
    ...
    private void forgiveMobAnger() {
        ...
        this.getWorld().getEntitiesByClass(MobEntity.class, lv, EntityPredicates.EXCEPT_SPECTATOR).stream().filter(entity -> entity instanceof Angerable).forEach(entity -> ((Angerable)((Object)entity)).forgive(this.getServerWorld(), this));
    }
    ...
    public TeleportTarget getRespawnTarget(boolean alive, TeleportTarget.PostDimensionTransition postDimensionTransition) {
        ...
        if (lv2 != null && lv != null) {
            ...
            if (optional.isPresent()) {
                ...
                return new TeleportTarget(lv2, lv3.pos(), Vec3d.ZERO, lv3.yaw(), 0.0f, postDimensionTransition);
            }
    ...
    private static Optional<RespawnPos> findRespawnPosition(ServerWorld world, BlockPos pos, float spawnAngle, boolean spawnForced, boolean alive) {
        ...
        if (lv2 instanceof BedBlock && BedBlock.isBedWorking(world)) {
            return BedBlock.findWakeUpPosition(EntityType.PLAYER, (CollisionView)world, pos, (Direction)lv.get(BedBlock.FACING), spawnAngle).map(respawnPos -> RespawnPos.fromCurrentPos(respawnPos, pos));
        }
        if (!spawnForced) {
    ...
    public ServerPlayerEntity teleportTo(TeleportTarget arg) {
        ...
        if (lv.getRegistryKey() == lv3) {
            this.networkHandler.requestTeleport(PlayerPosition.fromTeleportTarget(arg), arg.relatives());
    ...
    public Either<PlayerEntity.SleepFailureReason, Unit> trySleep(BlockPos pos) {
        ...
        if (!this.isCreative()) {
            ...
            List<HostileEntity> list = this.getWorld().getEntitiesByClass(HostileEntity.class, new Box(lv2.getX() - 8.0, lv2.getY() - 5.0, lv2.getZ() - 8.0, lv2.getX() + 8.0, lv2.getY() + 5.0, lv2.getZ() + 8.0), entity -> entity.isAngryAt(this.getServerWorld(), this));
            if (!list.isEmpty()) {
    ...
    protected void fall(double heightDifference, boolean onGround, BlockState state, BlockPos landedPosition) {
        if (this.spawnExtraParticlesOnFall && onGround && this.fallDistance > 0.0f) {
            ...
            this.getServerWorld().spawnParticles(new BlockStateParticleEffect(ParticleTypes.BLOCK, state), lv.x, lv.y, lv.z, i, 0.3f, 0.3f, 0.3f, 0.15f);
    ...
    public ServerCommandSource getCommandSource() {
        return new ServerCommandSource(this.getCommandOutput(), this.getPos(), this.getRotationClient(), this.getServerWorld(), this.getPermissionLevel(), this.getName().getString(), this.getDisplayName(), this.server, this);
    }
    ...
    public ItemEntity dropItem(ItemStack stack, boolean throwRandomly, boolean retainOwnership) {
        ItemEntity lv = this.dropPlayerItem(stack, throwRandomly, retainOwnership);
        if (lv == null) {
            return null;
        }
