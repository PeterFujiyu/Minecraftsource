...
public class PlaceCommand {
    ...
    public static int executePlaceStructure(ServerCommandSource source, RegistryEntry.Reference<Structure> structure, BlockPos pos) throws CommandSyntaxException {
        ...
        StructureStart lv4 = lv2.createStructureStart(structure, lv.getRegistryKey(), source.getRegistryManager(), lv3, lv3.getBiomeSource(), lv.getChunkManager().getNoiseConfig(), lv.getStructureTemplateManager(), lv.getSeed(), new ChunkPos(pos), 0, lv, biome -> true);
        if (!lv4.hasChildren()) {
        ...
        ChunkPos.stream(lv6, lv7).forEach(chunkPos -> lv4.place(lv, lv.getStructureAccessor(), lv3, lv.getRandom(), new BlockBox(chunkPos.getStartX(), lv.getBottomY(), chunkPos.getStartZ(), chunkPos.getEndX(), lv.getTopYInclusive() + 1, chunkPos.getEndZ()), (ChunkPos)chunkPos));
