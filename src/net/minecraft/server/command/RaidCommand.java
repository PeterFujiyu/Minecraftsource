...
public class RaidCommand {
    ...
    private static int executeSpawnLeader(ServerCommandSource source) {
        ...
        RaiderEntity lv = EntityType.PILLAGER.create(source.getWorld(), SpawnReason.COMMAND);
        if (lv == null) {
            source.sendError(Text.literal("Pillager failed to spawn"));
            return 0;
        }
        lv.setPatrolLeader(true);
        lv.equipStack(EquipmentSlot.HEAD, Raid.createOminousBanner(source.getRegistryManager().getOrThrow(RegistryKeys.BANNER_PATTERN)));
