...
public class Main {
        ...
        try {
            ...
            try {
                ...
                lv10 = (SaveLoader)Util.waitAndApply(applyExecutor -> SaveLoading.load(lv9, context -> {
                    ...
                    if (dynamic2 != null) {
                        ParsedSaveProperties lv2 = LevelStorage.parseSaveProperties(dynamic2, context.dataConfiguration(), (Registry<DimensionOptions>)lv, context.worldGenRegistryManager());
                    ...
                    if (optionSet.has(optionSpec3)) {
                        ...
                        lv3 = new LevelInfo(lv6.levelName, lv6.gameMode, lv6.hardcore, lv6.difficulty, false, new GameRules(context.dataConfiguration().enabledFeatures()), context.dataConfiguration());
                    ...
                    DimensionOptionsRegistryHolder.DimensionsConfig lv7 = lv5.toConfig((Registry<DimensionOptions>)lv);
                    Lifecycle lifecycle = lv7.getLifecycle().add(context.worldGenRegistryManager().getLifecycle());
    ...
    private static void forceUpgradeWorld(LevelStorage.Session session, DataFixer dataFixer, boolean eraseCache, BooleanSupplier continueCheck, DynamicRegistryManager dynamicRegistryManager, boolean recreateRegionFiles) {
        ...
        try (WorldUpdater lv = new WorldUpdater(session, dataFixer, dynamicRegistryManager, eraseCache, recreateRegionFiles);){
            Text lv2 = null;
            while (!lv.isDone()) {
                int i;
