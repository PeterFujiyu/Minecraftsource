...
public class LootContextTypes {
    ...
    public static final Codec<ContextType> CODEC = Identifier.CODEC.comapFlatMap(id -> Optional.ofNullable((ContextType)MAP.get(id)).map(DataResult::success).orElseGet(() -> DataResult.error(() -> "No parameter set exists with id: '" + String.valueOf(id) + "'")), MAP.inverse()::get);
    ...
    private static ContextType register(String name, Consumer<ContextType.Builder> type) {
        ContextType.Builder lv = new ContextType.Builder();
        type.accept(lv);
        ContextType lv2 = lv.build();
        Identifier lv3 = Identifier.ofVanilla(name);
        ContextType lv4 = MAP.put(lv3, lv2);
        if (lv4 != null) {
