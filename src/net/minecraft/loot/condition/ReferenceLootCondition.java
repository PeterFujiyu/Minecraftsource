...
public record ReferenceLootCondition(RegistryKey<LootCondition> id) implements LootCondition
{
    ...
    public void validate(LootTableReporter reporter) {
        if (!reporter.canUseReferences()) {
            reporter.report("Uses reference to " + String.valueOf(this.id.getValue()) + ", but references are not allowed");
            return;
        }
        ...
        reporter.getDataLookup().getOptionalEntry(this.id).ifPresentOrElse(entry -> ((LootCondition)entry.value()).validate(reporter.makeChild(".{" + String.valueOf(this.id.getValue()) + "}", this.id)), () -> reporter.report("Unknown condition table called " + String.valueOf(this.id.getValue())));
    }
