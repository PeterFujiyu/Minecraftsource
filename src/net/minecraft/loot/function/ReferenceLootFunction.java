...
extends ConditionalLootFunction {
    ...
    public void validate(LootTableReporter reporter) {
        if (!reporter.canUseReferences()) {
            reporter.report("Uses reference to " + String.valueOf(this.name.getValue()) + ", but references are not allowed");
            return;
        }
        ...
        reporter.getDataLookup().getOptionalEntry(this.name).ifPresentOrElse(arg2 -> ((LootFunction)arg2.value()).validate(reporter.makeChild(".{" + String.valueOf(this.name.getValue()) + "}", this.name)), () -> reporter.report("Unknown function table called " + String.valueOf(this.name.getValue())));
    }
