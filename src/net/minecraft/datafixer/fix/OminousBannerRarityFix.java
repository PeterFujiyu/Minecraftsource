...
extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        ...
        return TypeRewriteRule.seq(this.fixTypeEverywhereTyped("Ominous Banner block entity common rarity to uncommon rarity fix", type, typed -> {
            ...
            return object.equals("minecraft:banner") ? this.fixNameAndRarity((Typed<?>)typed, opticFinder2) : typed;
            ...
            return string.equals("minecraft:white_banner") ? this.fixNameAndRarity((Typed<?>)typed, opticFinder3) : typed;
        }));
    }

    private Typed<?> fixNameAndRarity(Typed<?> typed2, OpticFinder<?> opticFinder) {
        return typed2.updateTyped(opticFinder, typed -> typed.update(DSL.remainderFinder(), dynamic -> {
            boolean bl = dynamic.get("minecraft:item_name").asString().result().flatMap(TextFixes::getTranslate).filter(string -> string.equals("block.minecraft.ominous_banner")).isPresent();
            if (bl) {
                return dynamic.set("minecraft:rarity", dynamic.createString("uncommon")).set("minecraft:item_name", TextFixes.translate(dynamic.getOps(), "block.minecraft.ominous_banner"));
            }
            return dynamic;
        }));
    }
