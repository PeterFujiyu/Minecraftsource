...
extends DataFix {
    ...
    private Dynamic<?> fixEnchantments(Dynamic<?> tagDynamic) {
        Optional<Dynamic> optional = tagDynamic.get("ench").asStreamOpt().map(enchantments -> enchantments.map(enchantment -> enchantment.set("id", enchantment.createString(ID_TO_ENCHANTMENTS_MAP.getOrDefault(enchantment.get("id").asInt(0), "null"))))).map(tagDynamic::createList).result();
        if (optional.isPresent()) {
            tagDynamic = tagDynamic.remove("ench").set("Enchantments", optional.get());
        }
        return tagDynamic.update("StoredEnchantments", storedEnchantmentsDynamic -> DataFixUtils.orElse(storedEnchantmentsDynamic.asStreamOpt().map(storedEnchantments -> storedEnchantments.map(storedEnchantment -> storedEnchantment.set("id", storedEnchantment.createString(ID_TO_ENCHANTMENTS_MAP.getOrDefault(storedEnchantment.get("id").asInt(0), "null"))))).map(storedEnchantmentsDynamic::createList).result(), storedEnchantmentsDynamic));
    }
}
