...
extends ChoiceFix {
    ...
    protected Typed<?> transform(Typed<?> inputTyped) {
        ...
        Optional<Typed<?>> optional = inputTyped.getOptionalTyped(opticFinder2);
        Dynamic<?> dynamic = inputTyped.get(DSL.remainderFinder());
        if (optional.isEmpty() && dynamic.get("Saddle").asBoolean(false)) {
            Typed<?> typed2 = type.pointTyped(inputTyped.getOps()).orElseThrow(IllegalStateException::new);
            ...
            return inputTyped.set(opticFinder2, typed2).set(DSL.remainderFinder(), dynamic);
        }
        return inputTyped;
    }
}
