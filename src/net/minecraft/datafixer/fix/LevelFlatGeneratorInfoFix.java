...
extends DataFix {
    ...
    private Dynamic<?> fixGeneratorOptions(Dynamic<?> levelDynamic) {
        if (levelDynamic.get("generatorName").asString("").equalsIgnoreCase("flat")) {
            return levelDynamic.update(GENERATOR_OPTIONS_KEY, generatorOptionsDynamic -> DataFixUtils.orElse(generatorOptionsDynamic.asString().map(this::fixFlatGeneratorOptions).map(generatorOptionsDynamic::createString).result(), generatorOptionsDynamic));
        }
        return levelDynamic;
    }
