...
extends ChoiceFix {
    ...
    public Dynamic<?> fix(Dynamic<NbtElement> nbt) {
        ...
        Identifier lv = Replacements.REPLACEMENTS.get(Pair.of(optional.get(), optional2.get()));
        if (lv == null) {
            return nbt;
        }
        return nbt.set("normal_config", nbt.createString(lv.withSuffixedPath("/normal").toString())).set("ominous_config", nbt.createString(lv.withSuffixedPath("/ominous").toString()));
    }
    ...
    static final class Replacements {
        ...
        private static void register(Identifier id, String normal, String ominous) {
            try {
                NbtCompound lv = Replacements.parse(normal);
                NbtCompound lv2 = Replacements.parse(ominous);
                NbtCompound lv3 = lv.copy().copyFrom(lv2);
                NbtCompound lv4 = Replacements.removeDefaults(lv3.copy());
                Dynamic<NbtElement> dynamic = Replacements.toDynamic(lv);
                REPLACEMENTS.put(Pair.of(dynamic, Replacements.toDynamic(lv2)), id);
                REPLACEMENTS.put(Pair.of(dynamic, Replacements.toDynamic(lv3)), id);
                REPLACEMENTS.put(Pair.of(dynamic, Replacements.toDynamic(lv4)), id);
        ...
        private static Dynamic<NbtElement> toDynamic(NbtCompound nbt) {
            return new Dynamic<NbtElement>(NbtOps.INSTANCE, nbt);
        }
