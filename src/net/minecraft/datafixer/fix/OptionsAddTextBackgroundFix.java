...
extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped("OptionsAddTextBackgroundFix", this.getInputSchema().getType(TypeReferences.OPTIONS), optionsTyped -> optionsTyped.update(DSL.remainderFinder(), optionsDynamic -> DataFixUtils.orElse(optionsDynamic.get("chatOpacity").asString().map(string -> optionsDynamic.set("textBackgroundOpacity", optionsDynamic.createDouble(this.convertToTextBackgroundOpacity((String)string)))).result(), optionsDynamic)));
    }
