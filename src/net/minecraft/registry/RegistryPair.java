...
public record RegistryPair<T>(Optional<RegistryEntry<T>> entry, RegistryKey<T> key) {
    ...
    public static <T> Codec<RegistryPair<T>> createCodec(RegistryKey<Registry<T>> registryRef, Codec<RegistryEntry<T>> entryCodec) {
        return Codec.either(entryCodec, RegistryKey.createCodec(registryRef).comapFlatMap(arg -> DataResult.error(() -> "Cannot parse as key without registry"), Function.identity())).xmap(RegistryPair::create, RegistryPair::asEither);
    }

    public static <T> PacketCodec<RegistryByteBuf, RegistryPair<T>> createPacketCodec(RegistryKey<Registry<T>> registryRef, PacketCodec<RegistryByteBuf, RegistryEntry<T>> entryPacketCodec) {
    ...
    public Either<RegistryEntry<T>, RegistryKey<T>> asEither() {
        return this.entry.map(Either::left).orElseGet(() -> Either.right(this.key));
    }
