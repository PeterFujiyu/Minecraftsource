...
public class MathHelper {
    ...
    public static int hsvToArgb(float hue, float saturation, float value, int alpha) {
        ...
        return ColorHelper.getArgb(alpha, MathHelper.clamp((int)(o * 255.0f), 0, 255), MathHelper.clamp((int)(p * 255.0f), 0, 255), MathHelper.clamp((int)((switch (j) {
            case 0 -> {
                o = value;
                p = n;
                yield l;
    ...
    public static float lerpAngleRadians(float delta, float start, float end) {
        float i;
        for (i = end - start; i < (float)(-java.lang.Math.PI); i += (float)java.lang.Math.PI * 2) {
        }
