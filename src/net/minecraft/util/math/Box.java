...
public class Box {
    ...
    public static Optional<Vec3d> raycast(double minX, double minY, double minZ, double maxX, double maxY, double maxZ, Vec3d from, Vec3d to) {
        ...
        double j = to.x - from.x;
        double k = to.y - from.y;
        double l = to.z - from.z;
        Direction lv = Box.traceCollisionSide(minX, minY, minZ, maxX, maxY, maxZ, from, ds, null, j, k, l);
        if (lv == null) {
            return Optional.empty();
        }
        double m = ds[0];
        return Optional.of(from.add(m * j, m * k, m * l));
    }
    ...
    public boolean collides(Vec3d arg, List<Box> boundingBoxes) {
        Vec3d lv = this.getCenter();
        Vec3d lv2 = lv.add(arg);
        for (Box lv3 : boundingBoxes) {
            Box lv4 = lv3.expand(this.getLengthX() * 0.5, this.getLengthY() * 0.5, this.getLengthZ() * 0.5);
            if (lv4.contains(lv2) || lv4.contains(lv)) {
                return true;
            }
            if (!lv4.raycast(lv, lv2).isPresent()) continue;
            return true;
        }
        return false;
    }
