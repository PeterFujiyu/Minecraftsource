...
extends Vec3i {
    ...
    public static Iterable<BlockPos> iterate(Box box) {
        BlockPos lv = BlockPos.ofFloored(box.minX, box.minY, box.minZ);
        BlockPos lv2 = BlockPos.ofFloored(box.maxX, box.maxY, box.maxZ);
        return BlockPos.iterate(lv, lv2);
    }
    ...
    public static int iterateRecursively(BlockPos pos, int maxDepth, int maxIterations, BiConsumer<BlockPos, Consumer<BlockPos>> nextQueuer, Function<BlockPos, IterationState> callback) {
        ...
        while (!queue.isEmpty()) {
            ...
            long m = lv.asLong();
            if (!longSet.add(m) || (lv2 = callback.apply(lv)) == IterationState.SKIP) continue;
            if (lv2 == IterationState.STOP) break;
            if (++k >= maxIterations) {
                return k;
            }
