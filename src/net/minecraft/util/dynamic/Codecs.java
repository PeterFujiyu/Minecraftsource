...
public class Codecs {
    ...
    public static final Codec<Quaternionf> QUATERNION_F = Codec.FLOAT.listOf().comapFlatMap(list -> Util.decodeFixedLengthList(list, 4).map(listx -> new Quaternionf(((Float)listx.get(0)).floatValue(), ((Float)listx.get(1)).floatValue(), ((Float)listx.get(2)).floatValue(), ((Float)listx.get(3)).floatValue()).normalize()), quaternion -> List.of(Float.valueOf(quaternion.x), Float.valueOf(quaternion.y), Float.valueOf(quaternion.z), Float.valueOf(quaternion.w)));
    public static final Codec<AxisAngle4f> AXIS_ANGLE_4F = RecordCodecBuilder.create(instance -> instance.group(((MapCodec)Codec.FLOAT.fieldOf("angle")).forGetter(axisAngle -> Float.valueOf(axisAngle.angle)), ((MapCodec)VECTOR_3F.fieldOf("axis")).forGetter(axisAngle -> new Vector3f(axisAngle.x, axisAngle.y, axisAngle.z))).apply((Applicative<AxisAngle4f, ?>)instance, AxisAngle4f::new));
    ...
    public static final Codec<Integer> ARGB = Codec.withAlternative(Codec.INT, VECTOR_4F, vec4f -> ColorHelper.fromFloats(vec4f.w(), vec4f.x(), vec4f.y(), vec4f.z()));
    ...
    public static <I, E> Codec<E> idChecked(Codec<I> idCodec, Function<I, E> idToElement, Function<E, I> elementToId) {
        return idCodec.flatXmap(id -> {
            Object object2 = idToElement.apply(id);
            return object2 == null ? DataResult.error(() -> "Unknown element id: " + String.valueOf(id)) : DataResult.success(object2);
        }, element -> {
            Object object2 = elementToId.apply(element);
            if (object2 == null) {
    ...
    public static <E> Codec<List<E>> listOrSingle(Codec<E> entryCodec, Codec<List<E>> listCodec) {
        return Codec.either(listCodec, entryCodec).xmap(either -> either.map(list -> list, List::of), list -> list.size() == 1 ? Either.right(list.getFirst()) : Either.left(list));
    }
