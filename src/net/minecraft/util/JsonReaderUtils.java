...
public class JsonReaderUtils {
    ...
    public static <T> T parse(RegistryWrapper.WrapperLookup registries, com.mojang.brigadier.StringReader stringReader, Codec<T> codec) {
        JsonReader jsonReader = new JsonReader(new StringReader(stringReader.getRemaining()));
        jsonReader.setLenient(false);
        try {
            JsonElement jsonElement = Streams.parse(jsonReader);
            Object a = codec.parse(registries.getOps(JsonOps.INSTANCE), jsonElement).getOrThrow(JsonParseException::new);
            return (T)a;
