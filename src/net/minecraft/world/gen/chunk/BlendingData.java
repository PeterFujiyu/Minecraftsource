...
public class BlendingData {
    ...
    private int getSurfaceBlockY(Chunk chunk, int blockX, int blockZ) {
        int k = chunk.hasHeightmap(Heightmap.Type.WORLD_SURFACE_WG) ? Math.min(chunk.sampleHeightmap(Heightmap.Type.WORLD_SURFACE_WG, blockX, blockZ), this.oldHeightLimit.getTopYInclusive()) : this.oldHeightLimit.getTopYInclusive();
        int l = this.oldHeightLimit.getBottomY();
        BlockPos.Mutable lv = new BlockPos.Mutable(blockX, k, blockZ);
        while (lv.getY() > l) {
            if (SURFACE_BLOCKS.contains(chunk.getBlockState(lv).getBlock())) {
                return lv.getY();
            }
    ...
    private double[] calculateCollidableBlockDensityColumn(Chunk chunk, int chunkBlockX, int chunkBlockZ, int surfaceHeight) {
        ...
        Arrays.fill(ds, -1.0);
        BlockPos.Mutable lv = new BlockPos.Mutable(chunkBlockX, this.oldHeightLimit.getTopYInclusive() + 1, chunkBlockZ);
    ...
    public record Serialized(int minSection, int maxSection, Optional<double[]> heights) {
        ...
        public static final Codec<Serialized> CODEC = RecordCodecBuilder.create(instance -> instance.group(((MapCodec)Codec.INT.fieldOf("min_section")).forGetter(Serialized::minSection), ((MapCodec)Codec.INT.fieldOf("max_section")).forGetter(Serialized::maxSection), DOUBLE_ARRAY_CODEC.lenientOptionalFieldOf("heights").forGetter(Serialized::heights)).apply((Applicative<Serialized, ?>)instance, Serialized::new)).validate(Serialized::validate);
