...
public class GameRules {
    ...
    public static final Key<IntRule> SPAWN_CHUNK_RADIUS = GameRules.register("spawnChunkRadius", Category.MISC, IntRule.create(2, 0, 32, FeatureSet.empty(), (server, value) -> {
    ...
    public GameRules copy(FeatureSet enabledFeatures) {
        return new GameRules((Map)GameRules.streamAllRules(enabledFeatures).collect(ImmutableMap.toImmutableMap(Map.Entry::getKey, entry -> this.rules.containsKey(entry.getKey()) ? this.rules.get(entry.getKey()) : ((Type)entry.getValue()).createRule())), enabledFeatures);
    }
    ...
    public static class Type<T extends Rule<T>> {
        ...
        Type(Supplier<ArgumentType<?>> argumentType, Function<Type<T>, T> ruleFactory, BiConsumer<MinecraftServer, T> changeCallback, Acceptor<T> ruleAcceptor, FeatureSet requiredFeatures) {
    ...
    extends Rule<BooleanRule> {
        ...
        static Type<BooleanRule> create(boolean initialValue, BiConsumer<MinecraftServer, BooleanRule> changeCallback) {
            return new Type<BooleanRule>(BoolArgumentType::bool, type -> new BooleanRule((Type<BooleanRule>)type, initialValue), changeCallback, Visitor::visitBoolean, FeatureSet.empty());
        }
    ...
    extends Rule<IntRule> {
        ...
        private static Type<IntRule> create(int initialValue, BiConsumer<MinecraftServer, IntRule> changeCallback) {
            return new Type<IntRule>(IntegerArgumentType::integer, type -> new IntRule((Type<IntRule>)type, initialValue), changeCallback, Visitor::visitInt, FeatureSet.empty());
        }
        ...
        static Type<IntRule> create(int initialValue, int min, int max, FeatureSet requiredFeatures, BiConsumer<MinecraftServer, IntRule> changeCallback) {
            return new Type<IntRule>(() -> IntegerArgumentType.integer(min, max), type -> new IntRule((Type<IntRule>)type, initialValue), changeCallback, Visitor::visitInt, requiredFeatures);
        }
