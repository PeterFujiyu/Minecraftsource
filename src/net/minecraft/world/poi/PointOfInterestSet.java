...
public class PointOfInterestSet {
    ...
    private boolean add(PointOfInterest poi) {
        ...
        if (lv3 != null) {
            ...
            Util.logErrorOrPause("POI data mismatch: already registered at " + String.valueOf(lv));
        }
    ...
    public void updatePointsOfInterest(Consumer<BiConsumer<BlockPos, RegistryEntry<PointOfInterestType>>> updater) {
        if (!this.valid) {
            ...
            updater.accept((pos, poiEntry) -> {
                ...
                PointOfInterest lv = short2ObjectMap.computeIfAbsent(s2, s -> new PointOfInterest((BlockPos)pos, (RegistryEntry<PointOfInterestType>)poiEntry, this.updateListener));
                this.add(lv);
            });
            this.valid = true;
            this.updateListener.run();
    ...
    public record Serialized(boolean isValid, List<PointOfInterest.Serialized> records) {
        public static final Codec<Serialized> CODEC = RecordCodecBuilder.create(instance -> instance.group(Codec.BOOL.lenientOptionalFieldOf("Valid", false).forGetter(Serialized::isValid), ((MapCodec)PointOfInterest.Serialized.CODEC.listOf().fieldOf("Records")).forGetter(Serialized::records)).apply((Applicative<Serialized, ?>)instance, Serialized::new));

        public PointOfInterestSet toPointOfInterestSet(Runnable updateListener) {
            return new PointOfInterestSet(updateListener, this.isValid, this.records.stream().map(arg -> arg.toPointOfInterest(updateListener)).toList());
        }
    }
}
