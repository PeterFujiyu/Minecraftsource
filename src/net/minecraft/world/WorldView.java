...
BiomeAccess.Storage {
    ...
    default public boolean isSkyVisibleAllowingSea(BlockPos pos) {
        ...
        while (lv.getY() > pos.getY()) {
            ...
            if (lv2.getOpacity() > 0 && !lv2.isLiquid()) {
                return false;
            }
            lv = lv.down();
        }
    ...
    default public <T> RegistryWrapper<T> createCommandRegistryWrapper(RegistryKey<? extends Registry<? extends T>> registryRef) {
        RegistryWrapper.Impl lv = this.getRegistryManager().getOrThrow((RegistryKey)registryRef);
        return lv.withFeatureFilter(this.getEnabledFeatures());
    }
}
