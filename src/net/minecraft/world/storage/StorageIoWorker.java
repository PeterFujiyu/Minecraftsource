...
AutoCloseable {
    ...
    private final SequencedMap<ChunkPos, Result> results = new LinkedHashMap<ChunkPos, Result>();
    ...
    private boolean needsBlending(NbtCompound nbt) {
        if (!nbt.contains("DataVersion", NbtElement.NUMBER_TYPE) || nbt.getInt("DataVersion") < 4185) {
            return true;
        }
    ...
    public CompletableFuture<Void> setResult(ChunkPos pos, Supplier<NbtCompound> nbtSupplier) {
        return this.run(() -> {
            NbtCompound lv = (NbtCompound)nbtSupplier.get();
            Result lv2 = this.results.computeIfAbsent(pos, pos2 -> new Result(lv));
            lv2.nbt = lv;
            return lv2.future;
    ...
    public CompletableFuture<Optional<NbtCompound>> readChunkData(ChunkPos pos) {
        return this.run(() -> {
            ...
            if (lv != null) {
                return Optional.ofNullable(lv.copyNbt());
            }
            try {
