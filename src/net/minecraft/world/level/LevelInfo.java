...
public final class LevelInfo {
    ...
    public static LevelInfo fromDynamic(Dynamic<?> dynamic, DataConfiguration dataConfiguration) {
        ...
        return new LevelInfo(dynamic.get("LevelName").asString(""), lv, dynamic.get("hardcore").asBoolean(false), dynamic.get("Difficulty").asNumber().map(difficulty -> Difficulty.byId(difficulty.byteValue())).result().orElse(Difficulty.NORMAL), dynamic.get("allowCommands").asBoolean(lv == GameMode.CREATIVE), new GameRules(dataConfiguration.enabledFeatures(), dynamic.get("GameRules")), dataConfiguration);
    }
