...
extends HostileEntity {
    ...
    public boolean damage(ServerWorld world, DamageSource source, float amount) {
        ...
        if (!this.areSpikesRetracted() && !source.isIn(DamageTypeTags.AVOIDS_GUARDIAN_THORNS) && !source.isOf(DamageTypes.THORNS) && (entity = source.getSource()) instanceof LivingEntity) {
            ...
            lv.damage(world, this.getDamageSources().thorns(this), 2.0f);
        }
    ...
    extends Goal {
        ...
        public void tick() {
            ...
            if (this.beamTicks == 0) {
                ...
                ServerWorld lv2 = FireBeamGoal.getServerWorld(this.guardian);
                lv.damage(lv2, this.guardian.getDamageSources().indirectMagic(this.guardian, this.guardian), f);
                this.guardian.tryAttack(lv2, lv);
                this.guardian.setTarget(null);
            }
            super.tick();
        }
    ...
    implements TargetPredicate.EntityPredicate {
        ...
        @Override
        public boolean test(@Nullable LivingEntity target, ServerWorld world) {
