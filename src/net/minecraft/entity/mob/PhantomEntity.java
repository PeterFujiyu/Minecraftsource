...
implements Monster {
    ...
    extends LookControl {
        public PhantomLookControl(MobEntity arg) {
            super(arg);
        }
    ...
    extends Goal {
        ...
        public boolean canStart() {
            ...
            if (lv != null) {
                return PhantomEntity.this.testTargetPredicate(StartAttackGoal.castToServerWorld(PhantomEntity.this.getWorld()), lv, TargetPredicate.DEFAULT);
            }
            return false;
        }
    ...
    extends MovementGoal {
        ...
        public void tick() {
            ...
            if (PhantomEntity.this.getBoundingBox().expand(0.2f).intersects(lv.getBoundingBox())) {
                PhantomEntity.this.tryAttack(SwoopMovementGoal.castToServerWorld(PhantomEntity.this.getWorld()), lv);
                PhantomEntity.this.movementType = PhantomMovementType.CIRCLE;
    ...
    extends Goal {
        ...
        public boolean canStart() {
            ...
            ServerWorld lv = FindTargetGoal.castToServerWorld(PhantomEntity.this.getWorld());
            List<PlayerEntity> list = lv.getPlayers(this.PLAYERS_IN_RANGE_PREDICATE, PhantomEntity.this, PhantomEntity.this.getBoundingBox().expand(16.0, 64.0, 16.0));
            if (!list.isEmpty()) {
                list.sort(Comparator.comparing(Entity::getY).reversed());
                for (PlayerEntity lv2 : list) {
                    if (!PhantomEntity.this.testTargetPredicate(lv, lv2, TargetPredicate.DEFAULT)) continue;
                    PhantomEntity.this.setTarget(lv2);
                    return true;
                }
            }
            return false;
        ...
        public boolean shouldContinue() {
            LivingEntity lv = PhantomEntity.this.getTarget();
            if (lv != null) {
                return PhantomEntity.this.testTargetPredicate(FindTargetGoal.castToServerWorld(PhantomEntity.this.getWorld()), lv, TargetPredicate.DEFAULT);
            }
            return false;
        }
    }
