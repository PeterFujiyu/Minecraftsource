...
implements Hoglin {
    ...
    private static void addIdleTasks(Brain<ZoglinEntity> brain) {
        brain.setTaskList(Activity.IDLE, 10, ImmutableList.of(UpdateAttackTargetTask.create((world, target) -> target.getHoglinTarget(world)), LookAtMobWithIntervalTask.follow(8.0f, UniformIntProvider.create(30, 60)), new RandomTask(ImmutableList.of(Pair.of(StrollTask.create(0.4f), 2), Pair.of(GoToLookTargetTask.create(0.4f, 3), 2), Pair.of(new WaitTask(30, 60), 1)))));
    }
    ...
    private boolean shouldAttack(ServerWorld world, LivingEntity target) {
        EntityType<?> lv = target.getType();
        return lv != EntityType.ZOGLIN && lv != EntityType.CREEPER && Sensor.testAttackableTargetPredicate(world, this, target);
    }
    ...
    public boolean damage(ServerWorld world, DamageSource source, float amount) {
        ...
        if (this.canTarget(lv) && !TargetUtil.isNewTargetTooFar(this, lv, 4.0)) {
