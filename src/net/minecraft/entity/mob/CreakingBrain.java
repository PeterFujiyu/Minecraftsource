...
public class CreakingBrain {
    ...
    protected static final ImmutableList<? extends MemoryModuleType<?>> MEMORY_MODULES = ImmutableList.of(MemoryModuleType.MOBS, MemoryModuleType.VISIBLE_MOBS, MemoryModuleType.NEAREST_VISIBLE_PLAYER, MemoryModuleType.NEAREST_VISIBLE_TARGETABLE_PLAYER, MemoryModuleType.LOOK_TARGET, MemoryModuleType.WALK_TARGET, MemoryModuleType.CANT_REACH_WALK_TARGET_SINCE, MemoryModuleType.PATH, MemoryModuleType.ATTACK_TARGET, MemoryModuleType.ATTACK_COOLING_DOWN);
    ...
    static void addIdleTasks(Brain<CreakingEntity> brain) {
        brain.setTaskList(Activity.IDLE, 10, ImmutableList.of(UpdateAttackTargetTask.create((world, creaking) -> creaking.isActive(), (world, creaking) -> creaking.getBrain().getOptionalRegisteredMemory(MemoryModuleType.NEAREST_VISIBLE_TARGETABLE_PLAYER)), LookAtMobWithIntervalTask.follow(8.0f, UniformIntProvider.create(30, 60)), new RandomTask(ImmutableList.of(Pair.of(StrollTask.create(0.3f), 2), Pair.of(GoToLookTargetTask.create(0.3f, 3), 2), Pair.of(new WaitTask(30, 60), 1)))));
    }

    static void addFightTasks(Brain<CreakingEntity> brain) {
        brain.setTaskList(Activity.FIGHT, 10, ImmutableList.of(RangedApproachTask.create(1.0f), MeleeAttackTask.create(CreakingEntity::isUnrooted, 40), ForgetAttackTargetTask.create()), MemoryModuleType.ATTACK_TARGET);
    }
    ...
    public static Brain<CreakingEntity> create(Brain<CreakingEntity> brain) {
        ...
        CreakingBrain.addFightTasks(brain);
