...
implements VillagerDataContainer {
    ...
    public void writeCustomDataToNbt(NbtCompound nbt) {
        ...
        VillagerData.CODEC.encodeStart(NbtOps.INSTANCE, this.getVillagerData()).resultOrPartial(LOGGER::error).ifPresent(villagerData -> nbt.put("VillagerData", (NbtElement)villagerData));
    ...
    private void finishConversion(ServerWorld world) {
        this.convertTo(EntityType.VILLAGER, EntityConversionContext.create(this, false, false), villager -> {
            PlayerEntity lv3;
            for (EquipmentSlot lv : this.dropEquipment(world, stack -> !EnchantmentHelper.hasAnyEnchantmentsWith(stack, EnchantmentEffectComponentTypes.PREVENT_ARMOR_CHANGE))) {
