    ...
    private void roar() {
        if (this.isAlive()) {
            ...
            if (world instanceof ServerWorld) {
                ...
                List<Entity> list = this.getWorld().getEntitiesByClass(LivingEntity.class, this.getBoundingBox().expand(4.0), predicate);
                ...
                List<LivingEntity> list2 = this.getWorld().getEntitiesByClass(LivingEntity.class, this.getBoundingBox().expand(4.0), CAN_KNOCK_BACK_WITH_ROAR_ON_CLIENT);
                for (LivingEntity lv3 : list2) {
                    this.knockBack(lv3);
                }
                Vec3d lv4 = this.getBoundingBox().getCenter();
                for (int i = 0; i < 40; ++i) {
                    double d = this.random.nextGaussian() * 0.2;
                    double e = this.random.nextGaussian() * 0.2;
                    double f = this.random.nextGaussian() * 0.2;
                    this.getWorld().addParticle(ParticleTypes.POOF, lv4.x, lv4.y, lv4.z, d, e, f);
                }
            }
        }
    }
