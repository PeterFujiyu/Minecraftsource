...
public class PiglinBrain {
    ...
    private static List<ItemStack> getBarteredItem(PiglinEntity piglin) {
        ...
        ObjectArrayList<ItemStack> list = lv.generateLoot(new LootWorldContext.Builder((ServerWorld)piglin.getWorld()).add(LootContextParameters.THIS_ENTITY, piglin).build(LootContextTypes.BARTER));
        return list;
    }
    ...
    public static boolean isWearingPiglinSafeArmor(LivingEntity entity) {
        for (ItemStack lv : entity.getAllArmorItems()) {
            if (!lv.isIn(ItemTags.PIGLIN_SAFE_ARMOR)) continue;
            return true;
        }
        return false;
    }
