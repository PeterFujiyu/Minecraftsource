...
extends LivingEntity {
    ...
    private static final Map<EntityPose, EntityDimensions> POSE_DIMENSIONS = ImmutableMap.builder().put(EntityPose.STANDING, STANDING_DIMENSIONS).put(EntityPose.SLEEPING, SLEEPING_DIMENSIONS).put(EntityPose.GLIDING, EntityDimensions.changing(0.6f, 0.6f).withEyeHeight(0.4f)).put(EntityPose.SWIMMING, EntityDimensions.changing(0.6f, 0.6f).withEyeHeight(0.4f)).put(EntityPose.SPIN_ATTACK, EntityDimensions.changing(0.6f, 0.6f).withEyeHeight(0.4f)).put(EntityPose.CROUCHING, EntityDimensions.changing(0.6f, 1.5f).withEyeHeight(1.27f).withAttachments(EntityAttachments.builder().add(EntityAttachmentType.VEHICLE, VEHICLE_ATTACHMENT_POS))).put(EntityPose.DYING, EntityDimensions.fixed(0.2f, 0.2f).withEyeHeight(1.62f)).build();
    ...
    public static DefaultAttributeContainer.Builder createPlayerAttributes() {
        return LivingEntity.createLivingAttributes().add(EntityAttributes.ATTACK_DAMAGE, 1.0).add(EntityAttributes.MOVEMENT_SPEED, 0.1f).add(EntityAttributes.ATTACK_SPEED).add(EntityAttributes.LUCK).add(EntityAttributes.BLOCK_INTERACTION_RANGE, 4.5).add(EntityAttributes.ENTITY_INTERACTION_RANGE, 3.0).add(EntityAttributes.BLOCK_BREAK_SPEED).add(EntityAttributes.SUBMERGED_MINING_SPEED).add(EntityAttributes.SNEAKING_SPEED).add(EntityAttributes.MINING_EFFICIENCY).add(EntityAttributes.SWEEPING_DAMAGE_RATIO);
    }
    ...
    public void tick() {
        ...
        ItemStack lv2 = this.getMainHandStack();
        if (!ItemStack.areEqual(this.selectedItem, lv2)) {
            if (!ItemStack.areItemsEqual(this.selectedItem, lv2)) {
                this.resetLastAttackedTicks();
            }
            this.selectedItem = lv2.copy();
        }
    ...
    protected void updatePose() {
        ...
        EntityPose lv = this.isGliding() ? EntityPose.GLIDING : (this.isSleeping() ? EntityPose.SLEEPING : (this.isSwimming() ? EntityPose.SWIMMING : (this.isUsingRiptide() ? EntityPose.SPIN_ATTACK : (this.isSneaking() && !this.abilities.flying ? EntityPose.CROUCHING : EntityPose.STANDING))));
    ...
    protected void takeShieldHit(LivingEntity attacker) {
        ...
        if (attacker.disablesShield() && lv != null) {
            this.disableShield(lv);
        }
    }
    ...
    private boolean isSpaceAroundPlayerEmpty(double offsetX, double offsetZ, float f) {
        ...
        return this.getWorld().isSpaceEmpty(this, new Box(lv.minX + offsetX, lv.minY - (double)f - (double)1.0E-5f, lv.minZ + offsetZ, lv.maxX + offsetX, lv.minY, lv.maxZ + offsetZ));
    }
    ...
    public void attack(Entity target) {
        ...
        if (f > 0.0f || g > 0.0f) {
            ...
            if (bl52) {
                float k = this.getKnockbackAgainst(target, lv2) + (bl2 ? 1.0f : 0.0f);
                if (k > 0.0f) {
                    if (target instanceof LivingEntity) {
                ...
                if (bl42) {
                    ...
                    for (LivingEntity livingEntity : list) {
                        ...
                        livingEntity.serverDamage(lv2, m);
                        World world = this.getWorld();
                        if (!(world instanceof ServerWorld)) continue;
                        ServerWorld lv8 = (ServerWorld)world;
                        EnchantmentHelper.onTargetDamaged(lv8, livingEntity, lv2);
                    }
                ...
                if (target instanceof EnderDragonPart) {
                    lv9 = ((EnderDragonPart)target).owner;
                }
                boolean bl6 = false;
                World world = this.getWorld();
                if (world instanceof ServerWorld) {
                    ServerWorld lv10 = (ServerWorld)world;
                    if (lv9 instanceof LivingEntity) {
                        LivingEntity livingEntity = (LivingEntity)lv9;
                        bl6 = lv.postHit(livingEntity, this);
                    }
                    EnchantmentHelper.onTargetDamaged(lv10, target, lv2);
                }
                if (!this.getWorld().isClient && !lv.isEmpty() && lv9 instanceof LivingEntity) {
                    if (bl6) {
                        lv.postDamageEntity((LivingEntity)lv9, this);
                    }
                    if (lv.isEmpty()) {
                ...
                if (target instanceof LivingEntity) {
                    ...
                    if (this.getWorld() instanceof ServerWorld && n > 2.0f) {
                        ...
                        ((ServerWorld)this.getWorld()).spawnParticles(ParticleTypes.DAMAGE_INDICATOR, target.getX(), target.getBodyY(0.5), target.getZ(), n2, 0.1, 0.0, 0.1, 0.2);
                    }
                }
    ...
    public boolean handleFallDamage(float fallDistance, float damageMultiplier, DamageSource damageSource) {
        ...
        if (bl) {
            boolean bl22;
            h = Math.min(fallDistance, (float)(this.currentExplosionImpactPos.y - this.getY()));
            boolean bl3 = bl22 = h <= 0.0f;
            if (bl22) {
        ...
        if (h > 0.0f && super.handleFallDamage(h, damageMultiplier, damageSource)) {
            this.clearCurrentExplosion();
            return true;
        }
        return false;
