...
Nameable {
    ...
    public NbtList writeNbt(NbtList nbtList) {
        ...
        for (i = 0; i < this.main.size(); ++i) {
            ...
            nbtList.add(this.main.get(i).toNbt(this.player.getRegistryManager(), lv));
        }
        for (i = 0; i < this.armor.size(); ++i) {
            if (this.armor.get(i).isEmpty()) continue;
            lv = new NbtCompound();
            lv.putByte("Slot", (byte)(i + 100));
            nbtList.add(this.armor.get(i).toNbt(this.player.getRegistryManager(), lv));
        }
        for (i = 0; i < this.offHand.size(); ++i) {
            if (this.offHand.get(i).isEmpty()) continue;
            lv = new NbtCompound();
            lv.putByte("Slot", (byte)(i + 150));
            nbtList.add(this.offHand.get(i).toNbt(this.player.getRegistryManager(), lv));
        }
        return nbtList;
    }
