...
public interface InventoryOwner {
    ...
    default public void readInventory(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
        if (nbt.contains(INVENTORY_KEY, NbtElement.LIST_TYPE)) {
            this.getInventory().readNbtList(nbt.getList(INVENTORY_KEY, NbtElement.COMPOUND_TYPE), registries);
        }
    }

    default public void writeInventory(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
        nbt.put(INVENTORY_KEY, this.getInventory().toNbtList(registries));
    }
}
