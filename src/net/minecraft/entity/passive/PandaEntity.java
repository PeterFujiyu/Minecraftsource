...
extends AnimalEntity {
    ...
    public PassiveEntity createChild(ServerWorld world, PassiveEntity entity) {
        PandaEntity lv = EntityType.PANDA.create(world, SpawnReason.BREEDING);
        if (lv != null) {
    ...
    private void sneeze() {
        ...
        if (world instanceof ServerWorld && (lv4 = (ServerWorld)world).getGameRules().getBoolean(GameRules.DO_MOB_LOOT)) {
            this.forEachGiftedItem(lv4, LootTables.PANDA_SNEEZE_GAMEPLAY, this::dropStack);
        }
    }
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (!this.isBreedingItem(lv)) return ActionResult.PASS;
        ...
        if (this.isBaby()) {
            ...
            this.stop();
            this.setEating(true);
            ItemStack lv3 = this.getEquippedStack(EquipmentSlot.MAINHAND);
            if (!lv3.isEmpty() && !player.isInCreativeMode()) {
                this.dropStack(lv2, lv3);
            }
    ...
    extends Goal {
        ...
        public void start() {
            if (PandaEntity.this.getEquippedStack(EquipmentSlot.MAINHAND).isEmpty()) {
                List<ItemEntity> list = PandaEntity.this.getWorld().getEntitiesByClass(ItemEntity.class, PandaEntity.this.getBoundingBox().expand(8.0, 8.0, 8.0), PandaEntity::canEatFromGround);
                if (!list.isEmpty()) {
        ...
        public void stop() {
            ...
            if (!lv.isEmpty()) {
                PandaEntity.this.dropStack(PickUpFoodGoal.castToServerWorld(PandaEntity.this.getWorld()), lv);
