...
implements VariantHolder<Type> {
    ...
    protected void initGoals() {
        ...
        this.targetSelector.add(3, new DefendFriendGoal(LivingEntity.class, false, false, (entity, world) -> JUST_ATTACKED_SOMETHING_FILTER.test(entity) && !this.canTrust(entity.getUuid())));
    }
    ...
    public FoxEntity createChild(ServerWorld arg, PassiveEntity arg2) {
        FoxEntity lv = EntityType.FOX.create(arg, SpawnReason.BREEDING);
        if (lv != null) {
    ...
    extends AnimalMateGoal {
        ...
        protected void breed() {
            ...
            if (lv.getGameRules().getBoolean(GameRules.DO_MOB_LOOT)) {
    ...
    extends DiveJumpingGoal {
        ...
        public void tick() {
            ...
            if (lv != null && FoxEntity.this.distanceTo(lv) <= 2.0f) {
                FoxEntity.this.tryAttack(JumpChasingGoal.castToServerWorld(FoxEntity.this.getWorld()), lv);
    ...
    extends MoveToTargetPosGoal {
        ...
        protected void eatBerries() {
            if (!EatBerriesGoal.castToServerWorld(FoxEntity.this.getWorld()).getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
                return;
            }
    ...
    extends ActiveTargetGoal<LivingEntity> {
        ...
        public boolean canStart() {
            ...
            ServerWorld lv = DefendFriendGoal.castToServerWorld(FoxEntity.this.getWorld());
            for (UUID uUID : FoxEntity.this.getTrustedUuids()) {
                LivingEntity lv3;
                Entity lv2 = lv.getEntity(uUID);
                if (!(lv2 instanceof LivingEntity)) continue;
                this.friend = lv3 = (LivingEntity)lv2;
                this.offender = lv3.getAttacker();
                int i = lv3.getLastAttackedTime();
    ...
    extends Goal {
        ...
        protected boolean canCalmDown() {
            return !CalmDownGoal.castToServerWorld(FoxEntity.this.getWorld()).getTargets(LivingEntity.class, this.WORRIABLE_ENTITY_PREDICATE, FoxEntity.this, FoxEntity.this.getBoundingBox().expand(12.0, 6.0, 12.0)).isEmpty();
        }
    }
    ...
    implements TargetPredicate.EntityPredicate {
        ...
        public boolean test(LivingEntity target, ServerWorld world) {
            ...
            if (target instanceof ChickenEntity || target instanceof RabbitEntity || target instanceof HostileEntity) {
                return true;
            }
            ...
            if (FoxEntity.this.canTrust(target.getUuid())) {
                return false;
            }
            return !target.isSleeping() && !target.isSneaky();
        }
    }
}
