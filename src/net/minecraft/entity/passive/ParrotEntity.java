...
Flutterer {
    ...
    public void tickMovement() {
        if (this.songSource == null || !this.songSource.isWithinDistance(this.getPos(), 3.46) || !((World)this.getWorld()).getBlockState(this.songSource).isOf(Blocks.JUKEBOX)) {
            this.songPlaying = false;
            this.songSource = null;
        }
        if (((World)this.getWorld()).random.nextInt(400) == 0) {
            ParrotEntity.imitateNearbyMob((World)this.getWorld(), this);
        }
        super.tickMovement();
        this.flapWings();
    }
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (!this.isTamed() && lv.isIn(ItemTags.PARROT_FOOD)) {
            ...
            if (!this.isSilent()) {
                ((World)this.getWorld()).playSound(null, this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_PARROT_EAT, this.getSoundCategory(), 1.0f, 1.0f + (this.random.nextFloat() - this.random.nextFloat()) * 0.2f);
            }
    ...
    public SoundEvent getAmbientSound() {
        return ParrotEntity.getRandomSound((World)this.getWorld(), ((World)this.getWorld()).random);
    }
