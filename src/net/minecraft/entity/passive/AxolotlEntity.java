...
Bucketable {
    ...
    private void tickClient() {
        State lv = this.isPlayingDead() ? State.PLAYING_DEAD : (this.isInsideWaterOrBubbleColumn() ? State.IN_WATER : (this.isOnGround() ? State.ON_GROUND : State.IN_AIR));
    ...
    public boolean damage(ServerWorld world, DamageSource source, float amount) {
        ...
        if (!this.isAiDisabled() && this.getWorld().random.nextInt(3) == 0 && ((float)this.getWorld().random.nextInt(3) < amount || g / this.getMaxHealth() < 0.5f) && amount < g && this.isTouchingWater() && (source.getAttacker() != null || source.getSource() != null) && !this.isPlayingDead()) {
