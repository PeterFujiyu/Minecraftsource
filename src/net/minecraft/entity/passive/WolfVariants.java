...
public class WolfVariants {
    ...
    public static RegistryEntry<WolfVariant> fromBiome(DynamicRegistryManager dynamicRegistryManager, RegistryEntry<Biome> biome) {
        RegistryWrapper.Impl lv = dynamicRegistryManager.getOrThrow(RegistryKeys.WOLF_VARIANT);
        return lv.streamEntries().filter(entry -> ((WolfVariant)entry.value()).getBiomes().contains(biome)).findFirst().or(() -> ((Registry)lv).getOptional(DEFAULT)).or(((Registry)lv)::getDefaultEntry).orElseThrow();
    }
