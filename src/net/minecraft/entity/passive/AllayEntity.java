...
Vibrations {
    ...
    protected ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (!lv2.isEmpty() && hand == Hand.MAIN_HAND && lv.isEmpty()) {
            ...
            for (ItemStack lv4 : this.getInventory().clearToList()) {
                TargetUtil.give(this, lv4, this.getPos());
            }
    ...
    public void writeCustomDataToNbt(NbtCompound nbt) {
        ...
        Vibrations.ListenerData.CODEC.encodeStart(lv, this.vibrationListenerData).resultOrPartial(string -> LOGGER.error("Failed to encode vibration listener for Allay: '{}'", string)).ifPresent(encodedVibrationListenerData -> nbt.put("listener", (NbtElement)encodedVibrationListenerData));
    ...
    public void readCustomDataFromNbt(NbtCompound nbt) {
        ...
        if (nbt.contains("listener", NbtElement.COMPOUND_TYPE)) {
            Vibrations.ListenerData.CODEC.parse(lv, nbt.getCompound("listener")).resultOrPartial(string -> LOGGER.error("Failed to parse vibration listener for Allay: '{}'", string)).ifPresent(vibrationListenerData -> {
    ...
    private void duplicate() {
        AllayEntity lv = EntityType.ALLAY.create(this.getWorld(), SpawnReason.BREEDING);
        if (lv != null) {
