...
implements VariantHolder<RegistryEntry<CatVariant>> {
    ...
    public void readCustomDataFromNbt(NbtCompound nbt) {
        ...
        Optional.ofNullable(Identifier.tryParse(nbt.getString("variant"))).map(id -> RegistryKey.of(RegistryKeys.CAT_VARIANT, id)).flatMap(Registries.CAT_VARIANT::getOptional).ifPresent(this::setVariant);
    ...
    private void updateAnimations() {
        ...
        if (this.isInSleepingPose()) {
            ...
            List<PlayerEntity> list = this.getWorld().getNonSpectatingEntities(PlayerEntity.class, new Box(lv).expand(2.0, 2.0, 2.0));
    ...
    public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData) {
        ...
        if (lv2.getStructureAccessor().getStructureContaining(this.getBlockPos(), StructureTags.CATS_SPAWN_AS_BLACK).hasChildren()) {
            this.setVariant((RegistryEntry<CatVariant>)Registries.CAT_VARIANT.getOrThrow(CatVariant.ALL_BLACK));
            this.setPersistent();
        }
        return entityData;
    }
    ...
    extends Goal {
        ...
        public void stop() {
            ...
            if (this.owner.getSleepTimer() >= 100 && (double)f > 0.77 && (double)f < 0.8 && (double)((World)this.cat.getWorld()).getRandom().nextFloat() < 0.7) {
