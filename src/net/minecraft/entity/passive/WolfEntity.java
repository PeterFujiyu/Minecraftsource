...
VariantHolder<RegistryEntry<WolfVariant>> {
    ...
    public static final TargetPredicate.EntityPredicate FOLLOW_TAMED_PREDICATE = (entity, arg2) -> {
    ...
    protected void initDataTracker(DataTracker.Builder builder) {
        ...
        RegistryWrapper.Impl lv2 = lv.getOrThrow(RegistryKeys.WOLF_VARIANT);
        builder.add(VARIANT, (RegistryEntry)lv2.getOptional(WolfVariants.DEFAULT).or(((Registry)lv2)::getDefaultEntry).orElseThrow());
    ...
    public void readCustomDataFromNbt(NbtCompound nbt) {
        ...
        Optional.ofNullable(Identifier.tryParse(nbt.getString("variant"))).map(variantId -> RegistryKey.of(RegistryKeys.WOLF_VARIANT, variantId)).flatMap(variantKey -> this.getRegistryManager().getOrThrow(RegistryKeys.WOLF_VARIANT).getOptional(variantKey)).ifPresent(this::setVariant);
        if (nbt.contains("CollarColor", NbtElement.NUMBER_TYPE)) {
            this.setCollarColor(DyeColor.byId(nbt.getInt("CollarColor")));
        }
        this.readAngerFromNbt((World)this.getWorld(), nbt);
    }
    ...
    public void tickMovement() {
        super.tickMovement();
        if (!((World)this.getWorld()).isClient && this.furWet && !this.canShakeWaterOff && !this.isNavigating() && this.isOnGround()) {
            this.canShakeWaterOff = true;
            this.shakeProgress = 0.0f;
            this.lastShakeProgress = 0.0f;
            ((World)this.getWorld()).sendEntityStatus(this, EntityStatuses.SHAKE_OFF_WATER);
        }
    ...
    public void tick() {
        ...
        if (this.isWet()) {
            this.furWet = true;
            if (this.canShakeWaterOff && !((World)this.getWorld()).isClient) {
                ((World)this.getWorld()).sendEntityStatus(this, EntityStatuses.RESET_WOLF_SHAKE);
                this.resetShake();
            }
            ...
            if (this.shakeProgress > 0.4f) {
                ...
                for (int j = 0; j < i; ++j) {
                    ...
                    ((World)this.getWorld()).addParticle(ParticleTypes.SPLASH, this.getX() + (double)g, f + 0.8f, this.getZ() + (double)h, lv.x, lv.y, lv.z);
                }
            }
        }
    }
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (this.isTamed()) {
            ...
            if (lv.isOf(Items.SHEARS) && this.isOwner(player) && this.isWearingBodyArmor() && (!EnchantmentHelper.hasAnyEnchantmentsWith(this.getBodyArmor(), EnchantmentEffectComponentTypes.PREVENT_ARMOR_CHANGE) || player.isCreative())) {
                ...
                EntityView entityView = this.getWorld();
                if (!(entityView instanceof ServerWorld)) return ActionResult.SUCCESS;
                ServerWorld lv7 = (ServerWorld)entityView;
                this.dropStack(lv7, lv6);
                return ActionResult.SUCCESS;
            }
            ...
            ActionResult lv8 = super.interactMob(player, hand);
            if (lv8.isAccepted() || !this.isOwner(player)) return lv8;
        ...
        if (((World)this.getWorld()).isClient || !lv.isOf(Items.BONE) || this.hasAngerTime()) return super.interactMob(player, hand);
        lv.decrementUnlessCreative(1, player);
        this.tryTame(player);
    ...
    private void tryTame(PlayerEntity player) {
        if (this.random.nextInt(3) == 0) {
            ...
            ((World)this.getWorld()).sendEntityStatus(this, EntityStatuses.ADD_POSITIVE_PLAYER_REACTION_PARTICLES);
        } else {
            ((World)this.getWorld()).sendEntityStatus(this, EntityStatuses.ADD_NEGATIVE_PLAYER_REACTION_PARTICLES);
        }
    }
