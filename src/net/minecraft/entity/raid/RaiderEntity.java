...
extends PatrolEntity {
    ...
    extends Goal {
        ...
        public boolean canStart() {
            ...
            List<ItemEntity> list = ((Entity)this.actor).getWorld().getEntitiesByClass(ItemEntity.class, ((Entity)this.actor).getBoundingBox().expand(d, 8.0, d), OBTAINABLE_OMINOUS_BANNER_PREDICATE);
            for (ItemEntity lv : list) {
                long l = this.bannerItemCache.getOrDefault(lv.getId(), Long.MIN_VALUE);
                if (this.field_52512.getWorld().getTime() < l) {
                ...
                int2LongOpenHashMap.put(lv.getId(), this.field_52512.getWorld().getTime() + 600L);
            }
        ...
        private boolean shouldStop() {
            ...
            if (ItemStack.areEqual(((MobEntity)this.actor).getEquippedStack(EquipmentSlot.HEAD), Raid.createOminousBanner(((Entity)this.actor).getRegistryManager().getOrThrow(RegistryKeys.BANNER_PATTERN)))) {
                return true;
            }
        ...
        public void tick() {
            if (this.bannerItemEntity != null && this.bannerItemEntity.isInRange((Entity)this.actor, 1.414)) {
                ((RaiderEntity)this.actor).loot(PickUpBannerAsLeaderGoal.castToServerWorld(this.field_52512.getWorld()), this.bannerItemEntity);
            }
        }
    }
