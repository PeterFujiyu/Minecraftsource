...
TypeFilter<Entity, T> {
    ...
    public static final EntityType<CreakingEntity> CREAKING = EntityType.register("creaking", Builder.create(CreakingEntity::new, SpawnGroup.MONSTER).dimensions(0.9f, 2.7f).eyeHeight(2.3f).maxTrackingRange(8));
    ...
    public static final EntityType<PotionEntity> POTION = EntityType.register("potion", Builder.create(PotionEntity::new, SpawnGroup.MISC).dropsNothing().dimensions(0.25f, 0.25f).maxTrackingRange(4).trackingTickInterval(10));
    ...
    public static void loadFromEntityNbt(World world, @Nullable PlayerEntity player, @Nullable Entity entity, NbtComponent nbt) {
        ...
        EntityType<?> lv = nbt.getRegistryValueOfId(minecraftServer.getRegistryManager(), RegistryKeys.ENTITY_TYPE);
        if (entity.getType() != lv) {
            return;
        }
        if (!(world.isClient || !entity.getType().canPotentiallyExecuteCommands() || player != null && minecraftServer.getPlayerManager().isOperator(player.getGameProfile()))) {
            return;
        }
    ...
    public static Optional<Entity> getEntityFromNbt(NbtCompound nbt, World world, SpawnReason reason) {
        return Util.ifPresentOrElse(EntityType.fromNbt(nbt).map(type -> type.create(world, reason)), entity -> entity.readNbt(nbt), () -> LOGGER.warn("Skipping Entity with id {}", (Object)nbt.getString("id")));
    }
    ...
    public static Entity loadEntityWithPassengers(NbtCompound nbt, World world, SpawnReason reason, Function<Entity, Entity> entityProcessor) {
        return EntityType.loadEntityFromNbt(nbt, world, reason).map(entityProcessor).map(entity -> {
            if (nbt.contains("Passengers", NbtElement.LIST_TYPE)) {
                NbtList lv = nbt.getList("Passengers", NbtElement.COMPOUND_TYPE);
                for (int i = 0; i < lv.size(); ++i) {
                    Entity lv2 = EntityType.loadEntityWithPassengers(lv.getCompound(i), world, reason, entityProcessor);
    ...
    public static Stream<Entity> streamFromNbt(final List<? extends NbtElement> entityNbtList, final World world, final SpawnReason reason) {
        ...
        return StreamSupport.stream(new Spliterator<Entity>(){
            ...
            public boolean tryAdvance(Consumer<? super Entity> action) {
                return spliterator.tryAdvance((? super T nbt) -> EntityType.loadEntityWithPassengers((NbtCompound)nbt, world, reason, entity -> {
