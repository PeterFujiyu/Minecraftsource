...
public class EnderDragonFight {
    ...
    private void generateEndGateway(BlockPos pos) {
        ...
        this.world.getRegistryManager().getOptional(RegistryKeys.CONFIGURED_FEATURE).flatMap(arg -> arg.getOptional(EndConfiguredFeatures.END_GATEWAY_DELAYED)).ifPresent(arg2 -> ((ConfiguredFeature)arg2.value()).generate(this.world, this.world.getChunkManager().getChunkGenerator(), Random.create(), pos));
    }
    ...
    private EnderDragonEntity createDragon() {
        ...
        EnderDragonEntity lv = EntityType.ENDER_DRAGON.create(this.world, SpawnReason.EVENT);
        if (lv != null) {
            lv.setFight(this);
    ...
    public void crystalDestroyed(EndCrystalEntity enderCrystal, DamageSource source) {
        if (this.dragonSpawnState != null && this.crystals.contains(enderCrystal)) {
            ...
            if (lv instanceof EnderDragonEntity) {
                EnderDragonEntity lv2 = (EnderDragonEntity)lv;
                lv2.crystalDestroyed(this.world, enderCrystal, enderCrystal.getBlockPos(), source);
            }
        }
    }
