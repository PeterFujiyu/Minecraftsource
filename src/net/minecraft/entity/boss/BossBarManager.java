...
public class BossBarManager {
    ...
    public NbtCompound toNbt(RegistryWrapper.WrapperLookup registries) {
        ...
        for (CommandBossBar lv2 : this.commandBossBars.values()) {
            lv.put(lv2.getId().toString(), lv2.toNbt(registries));
        }
        return lv;
    }
    ...
    public void readNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
        for (String string : nbt.getKeys()) {
            Identifier lv = Identifier.of(string);
            this.commandBossBars.put(lv, CommandBossBar.fromNbt(nbt.getCompound(string), lv, registries));
        }
    }
