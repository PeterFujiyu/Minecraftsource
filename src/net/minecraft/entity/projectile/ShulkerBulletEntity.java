    ...
    public void tick() {
        ...
        if (!this.getWorld().isClient) {
            ...
            if (!(this.target == null || !this.target.isAlive() || this.target instanceof PlayerEntity && this.target.isSpectator())) {
                ...
                this.setVelocity(lv2.add((this.targetX - lv2.x) * 0.2, (this.targetY - lv2.y) * 0.2, (this.targetZ - lv2.z) * 0.2));
            } else {
                this.applyGravity();
            }
            lv = ProjectileUtil.getCollision(this, this::canHit);
        }
        lv2 = this.getVelocity();
        this.setPosition(this.getPos().add(lv2));
        this.tickBlockCollision();
        if (this.portalManager != null && this.portalManager.isInPortal()) {
            this.tickPortalTeleportation();
        }
        if (lv != null && this.isAlive() && lv.getType() != HitResult.Type.MISS) {
            this.hitOrDeflect(lv);
        }
        ProjectileUtil.setRotationFromVelocity(this, 0.5f);
        if (this.getWorld().isClient) {
            this.getWorld().addParticle(ParticleTypes.END_ROD, this.getX() - lv2.x, this.getY() - lv2.y + 0.15, this.getZ() - lv2.z, 0.0, 0.0, 0.0);
