...
extends ThrownItemEntity {
    ...
    protected void onCollision(HitResult hitResult) {
        ...
        if (lv3.matches(Potions.WATER)) {
            ...
            if (this.isLingering()) {
                this.applyLingeringPotion(lv3);
            } else {
                this.applySplashPotion(lv, lv3.getEffects(), hitResult.getType() == HitResult.Type.ENTITY ? ((EntityHitResult)hitResult).getEntity() : null);
            }
        }
    ...
    private void applySplashPotion(ServerWorld world, Iterable<StatusEffectInstance> effects, @Nullable Entity entity) {
        Box lv = this.getBoundingBox().expand(4.0, 2.0, 4.0);
        List<LivingEntity> list = world.getNonSpectatingEntities(LivingEntity.class, lv);
        if (!list.isEmpty()) {
            ...
            for (LivingEntity lv3 : list) {
                ...
                for (StatusEffectInstance lv4 : effects) {
                    ...
                    if (lv5.value().isInstant()) {
                        lv5.value().applyInstantEffect(world, this, this.getOwner(), lv3, lv4.getAmplifier(), e);
                        continue;
                    }
