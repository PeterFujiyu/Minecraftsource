...
extends Entity {
    ...
    public void tick() {
        ...
        if (world instanceof ServerWorld) {
            ...
            if (this.isAlive() || this.shouldDupe) {
                ...
                boolean bl2 = bl && this.getWorld().getFluidState(lv3).isIn(FluidTags.WATER);
                double d = this.getVelocity().lengthSquared();
                if (bl && d > 1.0 && (lv4 = this.getWorld().raycast(new RaycastContext(new Vec3d(this.prevX, this.prevY, this.prevZ), this.getPos(), RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.SOURCE_ONLY, this))).getType() != HitResult.Type.MISS && this.getWorld().getFluidState(lv4.getBlockPos()).isIn(FluidTags.WATER)) {
                    lv3 = lv4.getBlockPos();
                    bl2 = true;
                }
                if (this.isOnGround() || bl2) {
                    BlockState lv5 = this.getWorld().getBlockState(lv3);
                    this.setVelocity(this.getVelocity().multiply(0.7, -0.5, 0.7));
                    if (!lv5.isOf(Blocks.MOVING_PISTON)) {
                        if (!this.destroyedOnLanding) {
                            boolean bl5;
                            boolean bl3 = lv5.canReplace(new AutomaticItemPlacementContext(this.getWorld(), lv3, Direction.DOWN, ItemStack.EMPTY, Direction.UP));
                            boolean bl4 = FallingBlock.canFallThrough(this.getWorld().getBlockState(lv3.down())) && (!bl || !bl2);
                            boolean bl6 = bl5 = this.block.canPlaceAt(this.getWorld(), lv3) && !bl4;
                            if (bl3 && bl5) {
                                if (this.block.contains(Properties.WATERLOGGED) && this.getWorld().getFluidState(lv3).getFluid() == Fluids.WATER) {
                                    this.block = (BlockState)this.block.with(Properties.WATERLOGGED, true);
                                }
                                if (this.getWorld().setBlockState(lv3, this.block, Block.NOTIFY_ALL)) {
                                    BlockEntity lv6;
                                    ((ServerWorld)this.getWorld()).getChunkManager().chunkLoadingManager.sendToOtherNearbyPlayers(this, new BlockUpdateS2CPacket(lv3, this.getWorld().getBlockState(lv3)));
                                    this.discard();
                                    if (lv instanceof LandingBlock) {
                                        ((LandingBlock)((Object)lv)).onLanding(this.getWorld(), lv3, this.block, lv5, this);
                                    }
                                    if (this.blockEntityData != null && this.block.hasBlockEntity() && (lv6 = this.getWorld().getBlockEntity(lv3)) != null) {
                                        NbtCompound lv7 = lv6.createNbt(this.getWorld().getRegistryManager());
                                        for (String string : this.blockEntityData.getKeys()) {
                                            lv7.put(string, this.blockEntityData.get(string).copy());
                                        }
                                        try {
                                            lv6.read(lv7, this.getWorld().getRegistryManager());
                ...
                } else if (this.timeFalling > 100 && (lv3.getY() <= this.getWorld().getBottomY() || lv3.getY() > this.getWorld().getTopYInclusive()) || this.timeFalling > 600) {
                    if (this.dropItem && lv2.getGameRules().getBoolean(GameRules.DO_ENTITY_DROPS)) {
                        this.dropItem(lv2, lv);
                    }
                    this.discard();
    ...
    public boolean handleFallDamage(float fallDistance, float damageMultiplier, DamageSource damageSource) {
        ...
        this.getWorld().getOtherEntities(this, this.getBoundingBox(), predicate).forEach(entity -> entity.serverDamage(lv2, h));
