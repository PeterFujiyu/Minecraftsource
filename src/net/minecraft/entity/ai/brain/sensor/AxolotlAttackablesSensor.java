...
extends NearestVisibleLivingEntitySensor {
    ...
    protected boolean matches(ServerWorld world, LivingEntity entity, LivingEntity target) {
        return this.isInRange(entity, target) && target.isInsideWaterOrBubbleColumn() && (this.isAlwaysHostileTo(target) || this.canHunt(entity, target)) && Sensor.testAttackableTargetPredicate(world, entity, target);
    }
