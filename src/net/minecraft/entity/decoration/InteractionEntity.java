...
Targeter {
    ...
    protected void readCustomDataFromNbt(NbtCompound nbt) {
        ...
        if (nbt.contains(ATTACK_KEY)) {
            Interaction.CODEC.decode(NbtOps.INSTANCE, nbt.get(ATTACK_KEY)).resultOrPartial(Util.addPrefix("Interaction entity", LOGGER::error)).ifPresent(pair -> {
        ...
        if (nbt.contains(INTERACTION_KEY)) {
            Interaction.CODEC.decode(NbtOps.INSTANCE, nbt.get(INTERACTION_KEY)).resultOrPartial(Util.addPrefix("Interaction entity", LOGGER::error)).ifPresent(pair -> {
