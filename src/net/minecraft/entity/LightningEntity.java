...
extends Entity {
    ...
    private void spawnFire(int spreadAttempts) {
        ServerWorld lv;
        World world;
        if (this.cosmetic || !((world = this.getWorld()) instanceof ServerWorld) || !(lv = (ServerWorld)world).getGameRules().getBoolean(GameRules.DO_FIRE_TICK)) {
            return;
        }
        BlockPos lv2 = this.getBlockPos();
        BlockState lv3 = AbstractFireBlock.getState(this.getWorld(), lv2);
        if (this.getWorld().getBlockState(lv2).isAir() && lv3.canPlaceAt(this.getWorld(), lv2)) {
            this.getWorld().setBlockState(lv2, lv3);
        ...
        for (int j = 0; j < spreadAttempts; ++j) {
            ...
            lv3 = AbstractFireBlock.getState(this.getWorld(), lv4);
            if (!this.getWorld().getBlockState(lv4).isAir() || !lv3.canPlaceAt(this.getWorld(), lv4)) continue;
            this.getWorld().setBlockState(lv4, lv3);
            ++this.blocksSetOnFire;
        }
    }
