...
implements Leashable {
    ...
    public void tick() {
        ...
        if (!list.isEmpty()) {
            ...
            for (Entity lv3 : list) {
                ...
                if (bl && this.getPassengerList().size() < this.getMaxPassengers() && !lv3.hasVehicle() && this.isSmallerThanBoat(lv3) && lv3 instanceof LivingEntity && !(lv3 instanceof WaterCreatureEntity) && !(lv3 instanceof PlayerEntity) && !(lv3 instanceof CreakingEntity)) {
    ...
    public void applyLeashElasticity(Entity leashHolder, float distance) {
        Vec3d lv = leashHolder.getPos().subtract(this.getPos()).normalize().multiply((double)distance - 6.0);
        Vec3d lv2 = this.getVelocity();
        boolean bl = lv2.dotProduct(lv) > 0.0;
        this.setVelocity(lv2.add(lv.multiply(bl ? (double)0.15f : (double)0.2f)));
    }
    ...
    public ActionResult interact(PlayerEntity player, Hand hand) {
        ...
        if (!player.shouldCancelInteraction() && this.ticksUnderwater < 60.0f && (this.getWorld().isClient || player.startRiding(this))) {
