...
implements TestListener {
    ...
    public static void passTest(GameTestState test, String output) {
        StructureTestListener.placeAboveBeacon(test, Blocks.LIME_STAINED_GLASS);
    ...
    protected static void failTest(GameTestState test, Throwable output) {
        StructureTestListener.placeAboveBeacon(test, test.isRequired() ? Blocks.RED_STAINED_GLASS : Blocks.ORANGE_STAINED_GLASS);
    ...
    protected static void visualizeTest(GameTestState test, Block block) {
        ...
        BlockPos lv2 = StructureTestListener.getBeaconPos(test);
        lv.setBlockState(lv2, Blocks.BEACON.getDefaultState().rotate(test.getRotation()));
        StructureTestListener.placeAboveBeacon(test, block);
        for (int i = -1; i <= 1; ++i) {
            for (int j = -1; j <= 1; ++j) {
                BlockPos lv3 = lv2.add(i, -1, j);
                lv.setBlockState(lv3, Blocks.IRON_BLOCK.getDefaultState());
            }
        }
    }

    private static BlockPos getBeaconPos(GameTestState state) {
        BlockPos lv = state.getPos();
        BlockPos lv2 = new BlockPos(-1, -2, -1);
        return StructureTemplate.transformAround(lv.add(lv2), BlockMirror.NONE, state.getRotation(), lv);
    }

    private static void placeAboveBeacon(GameTestState state, Block block) {
        BlockPos lv2;
        ServerWorld lv = state.getWorld();
        if (lv.getBlockState(lv2 = StructureTestListener.getBeaconPos(state)).isOf(Blocks.BEACON)) {
            BlockPos lv3 = lv2.add(0, 1, 0);
            lv.setBlockState(lv3, block.getDefaultState());
        }
    }
