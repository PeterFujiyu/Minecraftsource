...
public class TestContext {
    ...
    public void expectEntityInside(EntityType<?> type, Box box) {
        ...
        if (list.isEmpty()) {
            throw new PositionedException("Expected " + type.getUntranslatedName(), BlockPos.ofFloored(lv.getCenter()), BlockPos.ofFloored(box.getCenter()), this.test.getTick());
        }
    }
    ...
    public void dontExpectEntityBetween(EntityType<?> type, Box box) {
        ...
        List<Entity> list = this.getWorld().getEntitiesByType(type, lv, Entity::isAlive);
        if (!list.isEmpty()) {
            throw new PositionedException("Did not expect " + type.getUntranslatedName(), BlockPos.ofFloored(lv.getCenter()), BlockPos.ofFloored(box.getCenter()), this.test.getTick());
        }
    }
    ...
    public <E extends Entity, T> void expectEntity(BlockPos pos, EntityType<E> type, Predicate<E> predicate) {
        ...
        for (Entity lv2 : list) {
            ...
            throw new GameTestException("Test failed for entity " + String.valueOf(lv2));
        }
    }
    ...
    public void expectContainerWith(BlockPos pos, Item item) {
        ...
        if (!(lv2 instanceof LockableContainerBlockEntity)) {
            Identifier lv3 = lv2 != null ? Registries.BLOCK_ENTITY_TYPE.getId(lv2.getType()) : null;
            throw new GameTestException("Expected a container at " + String.valueOf(pos) + ", found " + String.valueOf(lv3));
        }
