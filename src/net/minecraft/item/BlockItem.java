...
extends Item {
    ...
    public static boolean writeNbtToBlockEntity(World world, @Nullable PlayerEntity player, BlockPos pos, ItemStack stack) {
        ...
        if (!lv.isEmpty()) {
            BlockEntityType<?> lv2 = lv.getRegistryValueOfId(world.getRegistryManager(), RegistryKeys.BLOCK_ENTITY_TYPE);
            if (lv2 == null) {
                return false;
            }
            BlockEntity lv3 = world.getBlockEntity(pos);
            if (lv3 != null) {
                BlockEntityType<?> lv4 = lv3.getType();
                if (lv4 != lv2) {
                    return false;
                }
                if (lv4.canPotentiallyExecuteCommands() && (player == null || !player.isCreativeLevelTwoOp())) {
                    return false;
                }
                return lv.applyToBlockEntity(lv3, world.getRegistryManager());
            }
        }
        return false;
    }
    ...
    public boolean shouldShowOperatorBlockWarnings(ItemStack stack, @Nullable PlayerEntity player) {
        ...
        if (player != null && player.getPermissionLevel() >= 2 && (lv = stack.get(DataComponentTypes.BLOCK_ENTITY_DATA)) != null) {
            BlockEntityType<?> lv2 = lv.getRegistryValueOfId(player.getWorld().getRegistryManager(), RegistryKeys.BLOCK_ENTITY_TYPE);
