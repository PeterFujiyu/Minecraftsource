...
extends PersistentState {
    ...
    public static MapState fromNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
        ...
        RegistryOps<NbtElement> lv3 = registries.getOps(NbtOps.INSTANCE);
        ...
        for (int k = 0; k < lv5.size(); ++k) {
            ...
            lv2.addDecoration(MapDecorationTypes.FRAME, null, MapState.getFrameDecorationKey(lv6.getEntityId()), lv6.getPos().getX(), lv6.getPos().getZ(), lv6.getRotation(), null);
        }
        return lv2;
    }
    ...
    public NbtCompound writeNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
        Identifier.CODEC.encodeStart(NbtOps.INSTANCE, this.dimension.getValue()).resultOrPartial(LOGGER::error).ifPresent(dimension -> nbt.put("dimension", (NbtElement)dimension));
    ...
    public void update(PlayerEntity player, ItemStack stack) {
        ...
        for (int i = 0; i < this.updateTrackers.size(); ++i) {
            ...
            String string = lv3.getName().getString();
            ...
            } else if (!stack.isInFrame() && lv3.getWorld().getRegistryKey() == this.dimension && this.showDecorations) {
                this.addDecoration(MapDecorationTypes.PLAYER, lv3.getWorld(), string, lv3.getX(), lv3.getZ(), lv3.getYaw(), null);
            }
        ...
        if (stack.isInFrame() && this.showDecorations) {
            ...
            if (lv6 != null && lv4.getId() != lv6.getEntityId() && this.frames.containsKey(lv6.getKey())) {
                this.removeDecoration(MapState.getFrameDecorationKey(lv6.getEntityId()));
            }
            MapFrameMarker lv7 = new MapFrameMarker(lv5, lv4.getHorizontalFacing().getHorizontalQuarterTurns() * 90, lv4.getId());
            this.addDecoration(MapDecorationTypes.FRAME, player.getWorld(), MapState.getFrameDecorationKey(lv4.getId()), lv5.getX(), lv5.getZ(), lv4.getHorizontalFacing().getHorizontalQuarterTurns() * 90, null);
            this.frames.put(lv7.getKey(), lv7);
        }
        MapDecorationsComponent lv8 = stack.getOrDefault(DataComponentTypes.MAP_DECORATIONS, MapDecorationsComponent.DEFAULT);
        if (!this.decorations.keySet().containsAll(lv8.decorations().keySet())) {
            lv8.decorations().forEach((id, decoration) -> {
