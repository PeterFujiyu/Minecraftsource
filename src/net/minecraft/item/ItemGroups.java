...
public class ItemGroups {
    ...
    public static void collect() {
        ...
        for (RegistryKey<ItemGroup> lv : Registries.ITEM_GROUP.getKeys()) {
            ItemGroup lv2 = Registries.ITEM_GROUP.getValueOrThrow(lv);
    ...
    private static void addOminousBottles(ItemGroup.Entries entries, ItemGroup.StackVisibility visibility) {
        for (int i = 0; i <= 4; ++i) {
            ...
            lv.set(DataComponentTypes.OMINOUS_BOTTLE_AMPLIFIER, new OminousBottleAmplifierComponent(i));
    ...
    private static void addPaintings(ItemGroup.Entries entries, RegistryWrapper.WrapperLookup registries, RegistryWrapper.Impl<PaintingVariant> registryWrapper, Predicate<RegistryEntry<PaintingVariant>> filter, ItemGroup.StackVisibility stackVisibility) {
        ...
        registryWrapper.streamEntries().filter(filter).sorted(PAINTING_VARIANT_COMPARATOR).forEach(paintingVariantEntry -> {
            NbtComponent lv = NbtComponent.DEFAULT.with(lv, PaintingEntity.VARIANT_MAP_CODEC, paintingVariantEntry).getOrThrow().apply(nbt -> nbt.putString("id", "minecraft:painting"));
            ItemStack lv2 = new ItemStack(Items.PAINTING);
            lv2.set(DataComponentTypes.ENTITY_DATA, lv);
            entries.add(lv2, stackVisibility);
        });
    }
