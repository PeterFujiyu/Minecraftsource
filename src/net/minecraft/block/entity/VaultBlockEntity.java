...
extends BlockEntity {
    ...
    public NbtCompound toInitialChunkDataNbt(RegistryWrapper.WrapperLookup registries) {
        return Util.make(new NbtCompound(), nbt -> nbt.put("shared_data", VaultBlockEntity.encodeValue(VaultSharedData.codec, this.sharedData, registries)));
    }

    @Override
    protected void writeNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
        super.writeNbt(nbt, registries);
        nbt.put("config", VaultBlockEntity.encodeValue(VaultConfig.codec, this.config, registries));
        nbt.put("shared_data", VaultBlockEntity.encodeValue(VaultSharedData.codec, this.sharedData, registries));
        nbt.put("server_data", VaultBlockEntity.encodeValue(VaultServerData.codec, this.serverData, registries));
    }
    ...
    public static final class Server {
        ...
        private static ItemStack generateDisplayItem(ServerWorld world, BlockPos pos, RegistryKey<LootTable> lootTable) {
            ...
            ObjectArrayList<ItemStack> list = lv.generateLoot(lv2 = new LootWorldContext.Builder(world).add(LootContextParameters.ORIGIN, Vec3d.ofCenter(pos)).build(LootContextTypes.VAULT), world.getRandom());
            if (list.isEmpty()) {
        ...
        private static List<ItemStack> generateLoot(ServerWorld world, VaultConfig config, BlockPos pos, PlayerEntity player, ItemStack key) {
            ...
            LootWorldContext lv2 = new LootWorldContext.Builder(world).add(LootContextParameters.ORIGIN, Vec3d.ofCenter(pos)).luck(player.getLuck()).add(LootContextParameters.THIS_ENTITY, player).add(LootContextParameters.TOOL, key).build(LootContextTypes.VAULT);
