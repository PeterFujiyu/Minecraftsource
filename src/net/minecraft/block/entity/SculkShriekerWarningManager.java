...
public class SculkShriekerWarningManager {
    public static final Codec<SculkShriekerWarningManager> CODEC = RecordCodecBuilder.create(instance -> instance.group(((MapCodec)Codecs.NON_NEGATIVE_INT.fieldOf("ticks_since_last_warning")).orElse(0).forGetter(manager -> manager.ticksSinceLastWarning), ((MapCodec)Codecs.NON_NEGATIVE_INT.fieldOf("warning_level")).orElse(0).forGetter(manager -> manager.warningLevel), ((MapCodec)Codecs.NON_NEGATIVE_INT.fieldOf("cooldown_ticks")).orElse(0).forGetter(manager -> manager.cooldownTicks)).apply((Applicative<SculkShriekerWarningManager, ?>)instance, SculkShriekerWarningManager::new));
    ...
    private static List<ServerPlayerEntity> getPlayersInRange(ServerWorld world, BlockPos pos) {
        ...
        return world.getPlayers(player -> !player.isSpectator() && player.getPos().isInRange(lv, 16.0) && player.isAlive());
    }
