...
implements Clearable {
    ...
    public static void litServerTick(ServerWorld world, BlockPos pos, BlockState state, CampfireBlockEntity blockEntity, ServerRecipeManager.MatchGetter<SingleStackRecipeInput, CampfireCookingRecipe> recipeMatchGetter) {
        ...
        for (int i = 0; i < blockEntity.itemsBeingCooked.size(); ++i) {
            ...
            if (blockEntity.cookingTimes[i] < blockEntity.cookingTotalTimes[i] || !(lv3 = recipeMatchGetter.getFirstMatch(lv2 = new SingleStackRecipeInput(lv), world).map(recipe -> ((CampfireCookingRecipe)recipe.value()).craft(lv2, (RegistryWrapper.WrapperLookup)world.getRegistryManager())).orElse(lv)).isItemEnabled(world.getEnabledFeatures())) continue;
            ItemScatterer.spawn((World)world, (double)pos.getX(), (double)pos.getY(), (double)pos.getZ(), lv3);
