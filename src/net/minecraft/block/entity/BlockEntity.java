...
public abstract class BlockEntity {
    ...
    public final void read(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
        ...
        Components.CODEC.parse(registries.getOps(NbtOps.INSTANCE), nbt).resultOrPartial(error -> LOGGER.warn("Failed to load components: {}", error)).ifPresent(components -> {
    ...
    public final NbtCompound createNbt(RegistryWrapper.WrapperLookup registries) {
        ...
        Components.CODEC.encodeStart(registries.getOps(NbtOps.INSTANCE), this.components).resultOrPartial(snbt -> LOGGER.warn("Failed to save components: {}", snbt)).ifPresent(nbt -> lv.copyFrom((NbtCompound)nbt));
        return lv;
    }
    ...
    public static BlockEntity createFromNbt(BlockPos pos, BlockState state, NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
        ...
        return Registries.BLOCK_ENTITY_TYPE.getOptionalValue(lv).map(type -> {
            try {
