...
extends BlockEntity {
    ...
    private void generateItem(ServerWorld world, PlayerEntity player, ItemStack brush) {
        ...
        LootWorldContext lv3 = new LootWorldContext.Builder(world).add(LootContextParameters.ORIGIN, Vec3d.ofCenter(this.pos)).luck(player.getLuck()).add(LootContextParameters.THIS_ENTITY, player).add(LootContextParameters.TOOL, brush).build(LootContextTypes.ARCHAEOLOGY);
    ...
    private boolean readLootTableFromNbt(NbtCompound nbt) {
        if (nbt.contains(LOOT_TABLE_NBT_KEY, NbtElement.STRING_TYPE)) {
            this.lootTable = RegistryKey.of(RegistryKeys.LOOT_TABLE, Identifier.of(nbt.getString(LOOT_TABLE_NBT_KEY)));
    ...
    public NbtCompound toInitialChunkDataNbt(RegistryWrapper.WrapperLookup registries) {
        ...
        if (!this.item.isEmpty()) {
            lv.put(ITEM_NBT_KEY, this.item.toNbt(registries));
        }
        return lv;
    }
    ...
    protected void readNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
        ...
        this.item = !this.readLootTableFromNbt(nbt) && nbt.contains(ITEM_NBT_KEY) ? ItemStack.fromNbt(registries, nbt.getCompound(ITEM_NBT_KEY)).orElse(ItemStack.EMPTY) : ItemStack.EMPTY;
    ...
    protected void writeNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
        ...
        if (!this.writeLootTableToNbt(nbt) && !this.item.isEmpty()) {
            nbt.put(ITEM_NBT_KEY, this.item.toNbt(registries));
        }
    }
