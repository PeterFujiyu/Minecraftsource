...
Vibrations {
    ...
    protected void readNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
        ...
        if (nbt.contains("listener", NbtElement.COMPOUND_TYPE)) {
            Vibrations.ListenerData.CODEC.parse(lv, nbt.getCompound("listener")).resultOrPartial(string -> LOGGER.error("Failed to parse vibration listener for Sculk Shrieker: '{}'", string)).ifPresent(vibrationListener -> {
    ...
    protected void writeNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
        ...
        RegistryOps<NbtElement> lv = registries.getOps(NbtOps.INSTANCE);
        Vibrations.ListenerData.CODEC.encodeStart(lv, this.vibrationListenerData).resultOrPartial(string -> LOGGER.error("Failed to encode vibration listener for Sculk Shrieker: '{}'", string)).ifPresent(arg2 -> nbt.put("listener", (NbtElement)arg2));
    }
