...
extends FacingBlock {
    ...
    protected VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        return (state.get(SHORT) != false ? SHORT_HEAD_SHAPES : HEAD_SHAPES)[((Direction)state.get(FACING)).ordinal()];
    }
    ...
    public BlockState onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
        BlockPos lv;
        if (!world.isClient && player.getAbilities().creativeMode && this.isAttached(state, world.getBlockState(lv = pos.offset(((Direction)state.get(FACING)).getOpposite())))) {
