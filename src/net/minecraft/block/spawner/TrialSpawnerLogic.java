...
public final class TrialSpawnerLogic {
    ...
    public Codec<TrialSpawnerLogic> codec() {
        return RecordCodecBuilder.create(instance -> instance.group(TrialSpawnerConfig.ENTRY_CODEC.optionalFieldOf(NORMAL_CONFIG_NBT_KEY, RegistryEntry.of(TrialSpawnerConfig.DEFAULT)).forGetter(arg -> arg.normalConfig), TrialSpawnerConfig.ENTRY_CODEC.optionalFieldOf(OMINOUS_CONFIG_NBT_KEY, RegistryEntry.of(TrialSpawnerConfig.DEFAULT)).forGetter(arg -> arg.ominousConfig), TrialSpawnerData.codec.forGetter(TrialSpawnerLogic::getData), Codec.intRange(0, Integer.MAX_VALUE).optionalFieldOf("target_cooldown_length", 36000).forGetter(TrialSpawnerLogic::getCooldownLength), Codec.intRange(1, 128).optionalFieldOf("required_player_range", 14).forGetter(TrialSpawnerLogic::getDetectionRadius)).apply((Applicative<TrialSpawnerLogic, ?>)instance, (normalConfig, ominousConfig, data, cooldownLength, entityDetectionRange) -> new TrialSpawnerLogic((RegistryEntry<TrialSpawnerConfig>)normalConfig, (RegistryEntry<TrialSpawnerConfig>)ominousConfig, (TrialSpawnerData)data, (int)cooldownLength, (int)entityDetectionRange, this.trialSpawner, this.entityDetector, this.entitySelector)));
    }

    public TrialSpawnerLogic(TrialSpawner trialSpawner, EntityDetector entityDetector, EntityDetector.Selector entitySelector) {
        this(RegistryEntry.of(TrialSpawnerConfig.DEFAULT), RegistryEntry.of(TrialSpawnerConfig.DEFAULT), new TrialSpawnerData(), 36000, 14, trialSpawner, entityDetector, entitySelector);
    }

    public TrialSpawnerLogic(RegistryEntry<TrialSpawnerConfig> normalConfig, RegistryEntry<TrialSpawnerConfig> ominousConfig, TrialSpawnerData data, int cooldownLength, int entityDetectionRange, TrialSpawner trialSpawner, EntityDetector entityDetector, EntityDetector.Selector entitySelector) {
