...
public final class SaplingGenerator {
    ...
    public boolean generate(ServerWorld world, ChunkGenerator chunkGenerator, BlockPos pos, BlockState state, Random random) {
        ...
        if (lv != null && (lv2 = (RegistryEntry)world.getRegistryManager().getOrThrow(RegistryKeys.CONFIGURED_FEATURE).getOptional(lv).orElse(null)) != null) {
            for (int i = 0; i >= -1; --i) {
                for (int j = 0; j >= -1; --j) {
        ...
        RegistryEntry lv6 = world.getRegistryManager().getOrThrow(RegistryKeys.CONFIGURED_FEATURE).getOptional(lv5).orElse(null);
        if (lv6 == null) {
            return false;
        }
